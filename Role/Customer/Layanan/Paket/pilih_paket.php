<?php
session_start();
// Path dari Role/Customer/Layanan/Paket/ ke root projekPRGWEB/ = ../../../../koneksi.php
include '../../../../koneksi.php';
...
// Ambil Profil Customer untuk Header
$q_customer = sqlsrv_query($conn, "SELECT * FROM Pelanggan WHERE ID_Pelanggan = ? AND Status = 1 AND Is_Deleted = 0", [$id_customer]);
$d_customer = sqlsrv_fetch_array($q_customer, SQLSRV_FETCH_ASSOC);
if ($d_customer) { $d_customer = array_change_key_case($d_customer, CASE_LOWER); }
...
// =====================================================
// QUERY PAKET FOTO AKTIF
// =====================================================
$sql_paket = "SELECT ID_Paket, Nama_Paket, Durasi_Waktu, Harga_Paket, Deskripsi, Kapasitas_Orang, Foto_Paket 
              FROM Paket_Foto 
              WHERE Status = 1 AND Is_Deleted = 0 
              ORDER BY Harga_Paket ASC";
$query_paket = sqlsrv_query($conn, $sql_paket);

// Hitung total paket aktif
$total_paket = 0;
if ($query_paket !== false) {
    $sql_count = "SELECT COUNT(*) AS total FROM Paket_Foto WHERE Status = 1 AND Is_Deleted = 0";
    $res_count = sqlsrv_query($conn, $sql_count);
    if ($res_count !== false) {
        $row_count = sqlsrv_fetch_array($res_count, SQLSRV_FETCH_ASSOC);
        $total_paket = $row_count['total'] ?? 0;
    }
}
?>
...
<head>
    ...
    <title>Pilih Paket Foto - SpotLight Studio</title>
    ...
    <style>
        ...
        .paket-card-meta-item {
            display: flex; align-items: center; gap: 6px;
            font-size: 0.8rem; color: #4a5568; font-weight: 600;
        }
        .paket-card-meta-item i { color: var(--p-pink); font-size: 1rem; }
        .btn-pilih-paket {
...
<body>
                ...
                <li class="nav-item">
                    ...
                    <div class="submenu show" id="submenuLayanan">
                        <ul class="list-unstyled">
                            <li><a href="pilih_paket.php" class="submenu-link active"><i class="bi bi-collection-fill me-2"></i>Paket Foto</a></li>
    ...
    <div class="main-content">
        ...
        <div class="dashboard-header">
            <div>
                <h3 class="fw-bold mb-1">Pilih Paket Foto</h3>
                <p class="text-muted small mb-0">Langkah 1 dari 4 - Pilih paket foto yang sesuai dengan kebutuhan Anda.</p>
            </div>
        ...
        <div class="paket-grid">
            <?php
            if ($query_paket && sqlsrv_has_rows($query_paket)):
                while($row = sqlsrv_fetch_array($query_paket, SQLSRV_FETCH_ASSOC)):
                    ...
                    // Hitung jumlah ruangan tersedia untuk paket ini
                    $sql_ruangan_count = "SELECT COUNT(*) as total_ruangan FROM Paket_Ruangan WHERE ID_Paket = ?";
                    $res_ruangan = sqlsrv_query($conn, $sql_ruangan_count, [$row['ID_Paket']]);
                    $total_ruangan = 0;
                    if ($res_ruangan !== false) {
                        $row_ruangan = sqlsrv_fetch_array($res_ruangan, SQLSRV_FETCH_ASSOC);
                        $total_ruangan = $row_ruangan['total_ruangan'] ?? 0;
                    }
            ?>
                <div class="paket-card animate-fade-in-up" onclick="pilihPaket(<?= $row['ID_Paket'] ?>, '<?= htmlspecialchars(addslashes($row['Nama_Paket'])) ?>')">
                    <img src="<?= $img_src ?>" class="paket-card-img" alt="<?= htmlspecialchars($row['Nama_Paket']) ?>">
                    <div class="paket-card-body">
                        <div class="paket-card-badge">Paket Foto</div>
                        <div class="paket-card-title"><?= htmlspecialchars($row['Nama_Paket']) ?></div>
                        <div class="paket-card-desc"><?= htmlspecialchars($row['Deskripsi'] ?? 'Paket foto ' . $row['Nama_Paket'] . ' dengan kualitas terbaik.') ?></div>
                        <div class="paket-card-price">Rp <?= number_format($row['Harga_Paket'] ?? 0, 0, ',', '.') ?></div>
                        <div class="paket-card-meta">
                            <div class="paket-card-meta-item">
                                <i class="bi bi-stopwatch"></i>
                                <?= $row['Durasi_Waktu'] ?? 0 ?> menit
                            </div>
                            <div class="paket-card-meta-item">
                                <i class="bi bi-people-fill"></i>
                                <?= $row['Kapasitas_Orang'] ?? 0 ?> orang
                            </div>
                            <div class="paket-card-meta-item">
                        ...
                        <button class="btn-pilih-paket" id="btn-paket-<?= $row['ID_Paket'] ?>">
            ...
            ?>
                <div class="empty-state" style="grid-column: 1 / -1;">
                    ...
                    <p>Maaf, saat ini tidak ada paket foto yang aktif. Silakan kembali lagi nanti.</p>
                </div>
            <?php endif; ?>
        </div>
    ...
    <script>
        ...
        function pilihPaket(idPaket, namaPaket) {
            Swal.fire({
                ...
                text: 'Anda akan memilih paket "' + namaPaket + '"',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    window.location.href = '../Ruangan/pilih_ruangan.php?id_paket=' + idPaket;
                }
            });
        }
