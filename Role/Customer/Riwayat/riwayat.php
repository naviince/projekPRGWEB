...
<head>
    ...
    <title>Sesi Terjadwal – SpotLight Studio</title>
    ...
    <style>
        ...
        .sesi-item { display: flex; align-items: center; gap: 14px; padding: 20px; background: linear-gradient(135deg, #ffffff, #FFF0F3); border-radius: 16px; margin-bottom: 12px; transition: var(--transition-3d); border: 2px solid transparent; }
        ...
        .sesi-time { font-size: 0.9rem; font-weight: 700; color: var(--p-pink); }
        .sesi-title { font-weight: 700; font-size: 1rem; color: var(--text-dark); }
        .sesi-info { font-size: 0.85rem; color: var(--text-muted); }
        .countdown-badge { background: linear-gradient(135deg, #fffbeb, #fef3c7); color: #d97706; padding: 6px 14px; border-radius: 50px; font-size: 0.75rem; font-weight: 700; }
        .btn-action { background: linear-gradient(135deg, var(--p-pink), var(--d-pink)); color: #ffffff; border: none; border-radius: 12px; padding: 10px 20px; font-weight: 800; font-size: 0.85rem; transition: var(--transition-3d); text-decoration: none; display: inline-flex; align-items: center; gap: 8px; }
        ...
        .btn-action-success:hover { box-shadow: 0 8px 20px rgba(5, 150, 105, 0.3); }
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-1" style="font-size: 0.8rem;">
                        ...
                        <li class="breadcrumb-item active" style="color: var(--text-muted); font-weight: 600;">Sesi Terjadwal</li>
                    </ol>
                </nav>
                <h3 class="fw-bold mb-0">Sesi Terjadwal</h3>
                <p class="text-muted small mb-0">Sesi foto yang akan datang dan perlu diproses.</p>
            </div>
        </div>

        <div class="card-3d animate-fade-in">
            <div class="content-header">
                <h5 class="content-title"><i class="bi bi-camera-reels-fill text-danger me-2"></i>Daftar Sesi Menunggu</h5>
                <span class="countdown-badge"><?= $total_terjadwal ?> Sesi</span>
            </div>
                ...
                <div class="sesi-item"
                    ...
                    <div class="flex-grow-1">
                        <div class="d-flex justify-content-between align-items-start">
                            ...
                            <div class="text-end">
                                <?php if ($is_today): ?>
                                    <span class="badge" style="background: #fecaca; color: #dc2626; font-weight: 700; border-radius: 50px; font-size: 0.75rem;"><i class="bi bi-exclamation-circle me-1"></i>Hari Ini</span>
                                <?php elseif ($days_left > 0): ?>
                                    <span class="countdown-badge"><?= $days_left ?> hari lagi</span>
                                <?php endif; ?>
                            </div>
                        </div>
            ...
            <?php if (!$has_data): ?>
                <div class="text-center py-5">
                    <div style="width: 80px; height: 80px; background: linear-gradient(135deg, #f1f5f9, #e2e8f0); border-radius: 20px; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px;">
                        <i class="bi bi-calendar-check fs-1" style="color: #94a3b8;"></i>
                    </div>
                    <h6 class="fw-bold text-muted">Tidak Ada Sesi Terjadwal</h6>
                    <p class="text-muted" style="font-size: 0.85rem;">Semua sesi sudah selesai atau belum ada jadwal baru.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
        ...
        function openDetailModal(btn, e) {
            ...
            Swal.fire({
                ...
                html: '<div style="text-align:left;">' +
                      ...
                      '<p><strong>Waktu:</strong> ' + d.jamMulai + ' - ' + d.jamSelesai + ' (' + d.durasi + ' menit)</p>' +
                ...
                showDenyButton: true,
                denyButtonText: '<i class="bi bi-play-fill"></i> Mulai Sesi',
                denyButtonColor: '#059669'
            }).then((result) => {
                if (result.isDenied) {
                    window.location.href = '../Proses/index.php?id=' + d.id;
                }
            });
