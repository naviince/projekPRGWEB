...
.stats-scroll-wrapper{width:100%;overflow-x:auto;overflow-y:hidden;padding-bottom:10px;margin-bottom:10px;scrollbar-width:thin;scrollbar-color:var(--p-pink) #f1f5f9;}
...
.chart-card{background:#ffffff;border-radius:22px;border:1px solid rgba(255,228,233,0.8);box-shadow:0 8px 24px rgba(213,61,102,0.03);transition:var(--transition-3d);padding:25px;height:100%;}
.chart-card:hover{transform:translateY(-8px);box-shadow:0 20px 40px rgba(213,61,102,0.1);}
.chart-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;}
.chart-title{font-weight:700;font-size:1rem;color:var(--text-dark);}
.chart-badge{background:linear-gradient(135deg,var(--p-pink),var(--d-pink));color:#ffffff;padding:6px 14px;border-radius:8px;font-size:0.75rem;font-weight:700;}
.alert-item{display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid rgba(238,242,246,0.8);}
.alert-item:last-child{border-bottom:none;}
.alert-icon{width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0;}
...
.table-custom thead th{color:var(--text-muted);font-weight:800;font-size:0.7rem;text-transform:uppercase;letter-spacing:1px;padding:10px 15px;border:none;background:transparent;}
.table-custom tbody tr{background:#ffffff;box-shadow:0 2px 8px rgba(0,0,0,0.04);transition:var(--transition-3d);border-radius:12px;}
.table-custom tbody tr:hover{transform:translateX(4px);box-shadow:0 8px 20px rgba(213,61,102,0.08);}
.table-custom td{padding:14px 15px;font-size:0.85rem;font-weight:600;border:none;vertical-align:middle;}
...
<div class="stat-icon" style="width:40px;height:40px;background:var(--p-pink);color:#ffffff;font-size:1.1rem;border-radius:10px;"><i class="bi bi-broadcast"></i></div>
<div><h6 class="fw-bold mb-0" style="font-size:0.9rem;color:var(--p-pink);">Siaran Sistem SpotLight ✦</h6><small class="text-muted" style="font-size:0.8rem;font-weight:600;">Dashboard Admin - Kelola operasional studio dan verifikasi transaksi.</small></div>
</div>

<!-- HEADER -->
<div class="dashboard-header animate-fade-in delay-1">
<div><h3 class="fw-bold mb-1">Selamat Datang, <?= htmlspecialchars($nama_admin) ?>! 👋</h3><p class="text-muted small mb-0">Kelola data master, verifikasi pembayaran, dan pantau operasional studio.</p></div>
...
<div class="profile-header-btn shadow-sm" onclick="bukaModalBiodata()" title="Klik untuk melihat Biodata Anda"><img src="<?= $foto_admin_src ?>" alt="Admin Profil"></div>
</div>
</div>
...
<div class="stats-row">
<div class="stat-card-item"><div class="card-3d"><div class="stat-card"><div class="stat-icon stat-icon-pink"><i class="bi bi-person-fill-check"></i></div><div class="stat-content"><div class="stat-title">Pelanggan Aktif</div><div class="stat-val"><?= $total_pelanggan ?> Akun</div><div class="stat-subtitle">Terdaftar di sistem</div></div></div></div></div>
<div class="stat-card-item"><div class="card-3d"><div class="stat-card"><div class="stat-icon stat-icon-green"><i class="bi bi-calendar-plus-fill"></i></div><div class="stat-content"><div class="stat-title">Booking Hari Ini</div><div class="stat-val"><?= $booking_today ?> Order</div><div class="stat-subtitle">Masuk hari ini</div></div></div></div></div>
<div class="stat-card-item"><div class="card-3d"><div class="stat-card"><div class="stat-icon stat-icon-orange"><i class="bi bi-hourglass-split"></i></div><div class="stat-content"><div class="stat-title">Verifikasi DP</div><div class="stat-val"><?= $wait_dp ?> Pembayaran</div><div class="stat-subtitle">Menunggu konfirmasi</div></div></div></div></div>
<div class="stat-card-item"><div class="card-3d"><div class="stat-card"><div class="stat-icon stat-icon-cyan"><i class="bi bi-hourglass-bottom"></i></div><div class="stat-content"><div class="stat-title">Verifikasi Pelunasan</div><div class="stat-val"><?= $wait_lunas ?> Pembayaran</div><div class="stat-subtitle">Menunggu konfirmasi</div></div></div></div></div>
<div class="stat-card-item"><div class="card-3d"><div class="stat-card"><div class="stat-icon stat-icon-purple"><i class="bi bi-camera-fill"></i></div><div class="stat-content"><div class="stat-title">Sesi Terjadwal</div><div class="stat-val"><?= $sesi_terjadwal ?> Sesi</div><div class="stat-subtitle">Belum diproses</div></div></div></div></div>
...
<div class="alert-item"><div class="alert-icon alert-icon-danger"><i class="bi bi-box-seam"></i></div><div><div class="fw-bold" style="font-size:0.85rem;"><?= htmlspecialchars($row['Nama_Barang'])?></div><div class="text-muted" style="font-size:0.75rem;">Stok: <?= $row['Stok_Barang']?> (Min: <?= $row['Stok_Minimum']?>)</div></div></div>
<?php endwhile; endif;?>
<?php if($wait_dp>0):$has_alert=true;?>
<div class="alert-item"><div class="alert-icon alert-icon-warning"><i class="bi bi-credit-card"></i></div><div><div class="fw-bold" style="font-size:0.85rem;"><?= $wait_dp?> Pembayaran DP</div><div class="text-muted" style="font-size:0.75rem;">Menunggu verifikasi Admin</div></div></div>
<?php endif;?>
<?php if($wait_lunas>0):$has_alert=true;?>
<div class="alert-item"><div class="alert-icon alert-icon-info"><i class="bi bi-cash-stack"></i></div><div><div class="fw-bold" style="font-size:0.85rem;"><?= $wait_lunas?> Pelunasan</div><div class="text-muted" style="font-size:0.75rem;">Menunggu verifikasi Admin</div></div></div>
<?php endif;?>
<?php if(!$has_alert):?>
<div class="text-center py-4"><i class="bi bi-check-circle-fill fs-1 mb-2" style="color:#059669;"></i><p class="text-muted fw-bold mb-0" style="font-size:0.85rem;">Tidak ada peringatan</p><p class="text-muted" style="font-size:0.75rem;">Semua sistem berjalan normal</p></div>
<?php endif;?>
</div>
</div>
<div class="col-lg-8 animate-fade-in delay-2">
<div class="chart-card">
<div class="chart-header"><h5 class="chart-title"><i class="bi bi-graph-up-arrow text-danger me-2"></i>Tren Booking Bulanan</h5><span class="chart-badge">Tahun <?= date('Y')?></span></div>
<div style="height:300px;width:100%;"><canvas id="chartBooking"></canvas></div>
</div>
</div>
</div>
...
<div class="chart-card">
<div class="chart-header"><h5 class="chart-title"><i class="bi bi-bar-chart-fill text-danger me-2"></i>Distribusi Status Booking</h5></div>
<div style="height:280px;width:100%;"><canvas id="chartStatusBooking"></canvas></div>
</div>
</div>
<div class="col-lg-6 animate-fade-in delay-2">
<div class="chart-card">
<div class="chart-header"><h5 class="chart-title"><i class="bi bi-pie-chart-fill text-danger me-2"></i>Status Pembayaran</h5></div>
<div style="height:280px;width:100%;display:flex;align-items:center;justify-content:center;"><canvas id="chartPembayaran"></canvas></div>
</div>
</div>
</div>
...
<div class="chart-card">
<div class="chart-header"><h5 class="chart-title"><i class="bi bi-activity text-danger me-2"></i>Pembayaran Menunggu Verifikasi</h5><a href="../../Transaksi/Pembayaran/list.php" class="btn btn-sm" style="background:var(--s-pink);color:var(--p-pink);font-weight:700;border-radius:8px;font-size:0.75rem;text-decoration:none;">Verifikasi Semua</a></div>
<div class="table-responsive">
<table class="table-custom">
<thead><tr><th>Pelanggan</th><th>Tipe</th><th>Jumlah</th><th>Tanggal Upload</th><th>Status</th><th>Aksi</th></tr></thead>
<tbody>
...
<tr><td><div class="d-flex align-items-center gap-2"><div class="profile-table-avatar" style="width:32px;height:32px;border-radius:50%;overflow:hidden;"><img src="<?= $default_svg_avatar?>" alt="Avatar" style="width:100%;height:100%;object-fit:cover;"></div><span><?= htmlspecialchars($row['Nama_Pelanggan'])?></span></div></td><td><span class="badge-status <?= $tipe_badge?>"><?= $row['Tipe_Pembayaran']?></span></td><td class="fw-bold">Rp<?= number_format($row['Jumlah_Bayar'],0,',','.')?></td><td><?= $row['Tanggal_Upload']->format('d M Y H:i')?></td><td><span class="badge-status badge-menunggu">Menunggu</span></td><td><a href="../../Transaksi/Pembayaran/verifikasi.php?id=<?= $row['ID_Pembayaran']?>&aksi=terima" class="btn btn-sm" style="background:linear-gradient(135deg,var(--p-pink),var(--d-pink));color:#fff;border-radius:8px;font-weight:700;font-size:0.75rem;text-decoration:none;padding:6px 12px;"><i class="bi bi-check-lg me-1"></i>Verifikasi</a></td></tr>
<?php endwhile;else:?>
<tr><td colspan="6" class="text-center text-muted py-4"><i class="bi bi-check-circle-fill fs-1 mb-2" style="color:#059669;"></i><p class="fw-bold mb-0">Tidak ada pembayaran menunggu</p><p class="text-muted" style="font-size:0.75rem;">Semua pembayaran sudah diverifikasi</p></td></tr>
<?php endif;?>
</tbody>
</table>
</div>
...
<div class="profile-preview-box mx-auto" style="width:100px;height:100px;border:3px solid var(--s-pink);"><img src="<?= $foto_admin_src?>" alt="Foto Profil"></div>
...
gradientPink.addColorStop(0,'rgba(213,61,102,0.45)');gradientPink.addColorStop(1,'rgba(255,240,243,0.05)');
new Chart(ctxBooking,{type:'line',data:{labels:['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des'],datasets:[{label:'Booking',data:<?= json_encode($booking_bulan_data)?>,borderColor:'#D53D66',borderWidth:4,pointBackgroundColor:'#ffffff',pointBorderColor:'#D53D66',pointBorderWidth:3,pointRadius:6,pointHoverRadius:10,fill:true,backgroundColor:gradientPink,tension:0.4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{backgroundColor:'rgba(30,30,36,0.9)',titleFont:{family:'Plus Jakarta Sans',size:13},bodyFont:{family:'Plus Jakarta Sans',size:12},padding:12,cornerRadius:10}},scales:{y:{grid:{color:'rgba(255,228,233,0.4)'},ticks:{font:{family:'Plus Jakarta Sans',size:11}}},x:{grid:{display:false},ticks:{font:{family:'Plus Jakarta Sans',size:11}}}}}});

const ctxStatusBooking=document.getElementById('chartStatusBooking').getContext('2d');
new Chart(ctxStatusBooking,{type:'bar',data:{labels:['Menunggu DP','DP Terverifikasi','Menunggu Pelunasan','Lunas','Dibatalkan'],datasets:[{label:'Jumlah',data:[<?= $d_status_booking['menunggu_dp']??0?>,<?= $d_status_booking['dp_verified']??0?>,<?= $d_status_booking['tunggu_pelunasan']??0?>,<?= $d_status_booking['lunas']??0?>,<?= $d_status_booking['dibatalkan']??0?>],backgroundColor:['#fbbf24','#E85D84','#E85D84','#10b981','#ef4444'],borderRadius:10,borderSkipped:false}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{grid:{color:'rgba(255,228,233,0.4)'},ticks:{font:{family:'Plus Jakarta Sans'}}},x:{grid:{display:false},ticks:{font:{family:'Plus Jakarta Sans',size:10}}}}}});

const ctxPembayaran=document.getElementById('chartPembayaran').getContext('2d');
new Chart(ctxPembayaran,{type:'doughnut',data:{labels:['Menunggu','Valid','Tidak Valid'],datasets:[{data:[<?= $d_pembayaran_status['menunggu']??0?>,<?= $d_pembayaran_status['valid']??0?>,<?= $d_pembayaran_status['tidak_valid']??0?>],backgroundColor:['#fbbf24','#10b981','#ef4444'],borderWidth:0,hoverOffset:10}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{boxWidth:12,font:{family:'Plus Jakarta Sans',weight:'600'},padding:20}}},cutout:'65%'}});
</script>
