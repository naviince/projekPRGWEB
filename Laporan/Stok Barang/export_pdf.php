...
function validatePassword($pass) { 
    return strlen($pass) >= 8 && preg_match('/[A-Za-z]/', $pass) && preg_match('/[0-9]/', $pass) && preg_match('/[^A-Za-z0-9]/', $pass); 
}

function validatePhone($hp) { 
    return preg_match('/^\+62[0-9]{9,13}$/', $hp); 
}
...
function cekDuplikat($conn, $field, $value) { 
    return safe_sqlsrv_count($conn, "SELECT COUNT(*) AS total FROM Karyawan WHERE $field = ? AND Is_Deleted = 0", array($value)); 
}
...
// =====================================================
// PROSES TAMBAH — MENGGUNAKAN SP sp_Karyawan_Create
// =====================================================
$error_crud = "";
$error_fields = array();
...
if (isset($_POST['tambah_karyawan'])) {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        // =====================================================
        // VALIDASI PER FIELD (sama seperti asli)
        // =====================================================
        if (empty($nik)) { 
            $errors[] = "NIK wajib diisi!"; 
            $error_fields['nik'] = true; 
        ...
        if (empty($email)) { 
            ...
            $errors[] = "Format email tidak valid! Contoh: nama@email.com"; 
            $error_fields['email'] = true; 
        }
        ...
        if (empty($pass)) { 
            ...
            $errors[] = "Password minimal 8 karakter, harus mengandung huruf, angka, dan simbol!"; 
            $error_fields['password'] = true; 
        } elseif ($pass !== $pass_confirm) { 
            $errors[] = "Password dan konfirmasi password tidak cocok!"; 
            $error_fields['password_confirm'] = true; 
        }

        if (empty($hp_raw)) { 
            $errors[] = "Nomor telepon wajib diisi!"; 
            $error_fields['no_hp'] = true; 
        } elseif (!preg_match('/^[0-9]{9,13}$/', $hp_raw)) { 
            $errors[] = "No HP harus 9-13 digit angka (tanpa +62)!"; 
            $error_fields['no_hp'] = true; 
        } elseif (!validatePhone($hp)) { 
            $errors[] = "No HP tidak valid! Format: +62[9-13 digit]"; 
            $error_fields['no_hp'] = true; 
        }

        if (empty($dob)) { 
            $errors[] = "Tanggal lahir wajib diisi!"; 
            $error_fields['tanggal_lahir'] = true; 
        } elseif (!validateTanggal($dob)) { 
            $errors[] = "Tanggal lahir tidak valid! Format: YYYY-MM-DD, tidak boleh di masa depan."; 
            $error_fields['tanggal_lahir'] = true; 
        } elseif ($umur < 17) { 
            $errors[] = "Umur minimal 17 tahun! (Umur saat ini: $umur tahun)"; 
            $error_fields['tanggal_lahir'] = true; 
        } elseif ($umur > 60) { 
            $errors[] = "Umur maksimal 60 tahun! (Umur saat ini: $umur tahun)"; 
            $error_fields['tanggal_lahir'] = true; 
        }

        if (empty($jk)) { 
            $errors[] = "Jenis kelamin wajib dipilih!"; 
            $error_fields['jenis_kelamin'] = true; 
        ...
        if (empty($role)) { 
            ...
            $error_fields['role_karyawan'] = true; 
        } elseif (!in_array($role, ['Admin','Fotografer','Owner'])) { 
        ...
        if (empty($alamat)) { 
            ...
            $error_fields['alamat'] = true; 
        }

        // =====================================================
        // CEK DUPLIKAT (sama seperti asli)
        // =====================================================
        if (empty($errors)) {
            if (cekDuplikat($conn, 'NIK', $nik) > 0) { 
                $errors[] = "NIK sudah terdaftar!"; 
                $error_fields['nik'] = true; 
            }
            if (safe_sqlsrv_count($conn, "SELECT COUNT(*) AS total FROM Karyawan WHERE LOWER(Username_Karyawan) = LOWER(?) AND Is_Deleted = 0", array($username)) > 0) { 
                $errors[] = "Username sudah digunakan!"; 
                $error_fields['username'] = true; 
            }
            if (safe_sqlsrv_count($conn, "SELECT COUNT(*) AS total FROM Karyawan WHERE LOWER(Email_Karyawan) = LOWER(?) AND Is_Deleted = 0", array($email)) > 0) { 
                $errors[] = "Email sudah terdaftar!"; 
                $error_fields['email'] = true; 
            }
            if (cekDuplikat($conn, 'No_Hp', $hp) > 0) { 
        ...
        // =====================================================
        // CEK OWNER HANYA 1 (sama seperti asli)
        // =====================================================
        if (empty($errors) && $role === 'Owner') {
            ...
            if ($owner_count > 0) { 
                $errors[] = "Sudah ada Owner dalam sistem! Hanya boleh 1 Owner."; 
                $error_fields['role_karyawan'] = true; 
            }
        }

        // =====================================================
        // UPLOAD FOTO (sama seperti asli)
        // =====================================================
        ...
        if (empty($errors) && isset($_FILES['foto_profil']) && $_FILES['foto_profil']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($mime_type, $allowed)) { 
                ...
                if (!$img_info) { 
                    ...
                    if (!file_exists('../../assets/img/karyawan/')) {
                        mkdir('../../assets/img/karyawan/', 0777, true);
                    }
                    ...
                    if (extension_loaded('gd') && function_exists('imagecreatetruecolor')) {
                        ...
                        if ($mime_type == 'image/png') {
                            imagepng($thumb, $upload_path, 8);
                        } else {
                            imagejpeg($thumb, $upload_path, 85);
                        }
        ...
        // =====================================================
        // EKSEKUSI SP sp_Karyawan_Create
        // =====================================================
        if (empty($errors)) {
            // Hash password di PHP sebelum kirim ke SP
            $pass_hash = password_hash($pass, PASSWORD_BCRYPT);
            
            // PANGGIL SP — sesuai dengan sp_Karyawan_Create
            $sql_sp = "EXEC sp_Karyawan_Create 
                @NIK = ?, 
                @Nama_Karyawan = ?, 
                @Username_Karyawan = ?, 
                @Email_Karyawan = ?, 
            ...
            $params_sp = array(
                ...
                $pass_hash,     // @Password_Karyawan (sudah di-hash)
                $jk,            // @Jenis_Kelamin
                $dob,           // @Tanggal_Lahir
                $role,          // @Role_Karyawan
                $hp,            // @No_Hp
                ...
                $username_session // @Created_By
            );
            
            $stmt_sp = safe_sqlsrv_query($conn, $sql_sp, $params_sp);
            
            if ($stmt_sp) {
                // SP berhasil dieksekusi
                header("Location: index.php?status_sukses=tambah");
                exit();
            } else {
                // SP gagal — kemungkinan constraint DB atau error lain
                $sqlsrv_errors = sqlsrv_errors(SQLSRV_ERR_ALL);
                $db_error_msg = "Gagal menyimpan data ke database!";
                
                // Cek pesan error dari SQL Server
                if (!empty($sqlsrv_errors)) {
                    foreach ($sqlsrv_errors as $error) {
                        $msg = $error['message'] ?? '';
                        // Cek constraint violation
                        if (strpos($msg, 'CHK_Karyawan_Password') !== false) {
                            $db_error_msg = "Password tidak memenuhi syarat database!";
                        } elseif (strpos($msg, 'UNIQUE') !== false || strpos($msg, 'duplicate') !== false) {
                            $db_error_msg = "Data sudah ada (duplikat)!";
                        } elseif (strpos($msg, 'CHK_') !== false) {
                            $db_error_msg = "Data tidak valid menurut aturan database!";
                        }
                    }
                }
                ...
                // Hapus foto yang sudah diupload jika gagal
                if ($foto_profil != 'default.jpg') {
                    @unlink('../../assets/img/karyawan/' . $foto_profil);
                }
            }
        }
...
function hasError($field, $error_fields) { 
    return isset($error_fields[$field]) && $error_fields[$field]; 
}
?>
<!DOCTYPE html>
<html lang="id">
