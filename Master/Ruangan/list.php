...
$list_sql = "SELECT 
    r.ID_Ruangan, r.Nama_Ruangan, r.Deskripsi, r.Foto_Ruangan, r.Status, r.Is_Deleted,
...
FROM Ruangan r {$where_sql} ORDER BY {$order_clause} OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
...
.ruangan-preview{width:70px;height:70px;object-fit:cover;border-radius:16px;border:2px solid var(--light-pink);transition:var(--t);flex-shrink:0}
...
.btn-action-toggle:hover{background:#059669;color:#fff;transform:translateY(-2px)}
.btn-action-toggle-off{color:#718096;border-color:#e2e8f0}
.btn-action-toggle-off:hover{background:#718096;color:#fff;transform:translateY(-2px)}
.btn-action-soft-delete{color:#dc2626;border-color:#fee2e2}
.btn-action-soft-delete:hover{background:#dc2626;color:#fff;transform:translateY(-2px)}
...
.btn-action-hard-delete:hover{background:#7c2d12;color:#fff;transform:translateY(-2px)}
...
.modal-detail .modal-body{padding:30px;background:#f8fafc}
.modal-detail .detail-foto{width:100%;height:200px;object-fit:cover;border-radius:16px;border:3px solid var(--light-pink);margin-bottom:20px}
.modal-detail .detail-info-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-bottom:20px}
.modal-detail .detail-info-item{background:#fff;border-radius:14px;padding:16px;border:2px solid #e2e8f0;text-align:center}
.modal-detail .detail-info-item .detail-icon{font-size:1.5rem;color:var(--p-pink);margin-bottom:8px}
.modal-detail .detail-info-item .detail-label{font-size:0.7rem;font-weight:800;color:#94a3b8;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:4px}
.modal-detail .detail-info-item .detail-value{font-size:1.2rem;font-weight:800;color:var(--text-dark)}
.modal-detail .detail-section-title{font-weight:800;font-size:0.9rem;color:var(--text-dark);margin-bottom:12px;display:flex;align-items:center;gap:8px;text-transform:uppercase;letter-spacing:0.8px}
.modal-detail .detail-section-title i{color:var(--p-pink)}
.modal-detail .detail-paket-card{background:#fff;border-radius:14px;border:2px solid #e2e8f0;padding:14px;display:flex;align-items:center;gap:12px;margin-bottom:8px}
...
.modal-detail .detail-paket-info{flex:1}
.modal-detail .detail-paket-nama{font-weight:700;font-size:0.9rem;color:var(--text-dark)}
...
.modal-detail .detail-badge{background:linear-gradient(135deg,#f8fafc,#f1f5f9);border:2px solid #e2e8f0;border-radius:10px;padding:8px 14px;font-weight:600;font-size:0.8rem;color:#4a5568;display:flex;align-items:center;gap:6px}
...
<p class="text-muted small mb-0">Kelola data ruangan studio. Kapasitas diatur oleh Paket Foto.</p>
</div>
...
<div class="stat-card-item"><div class="card-3d"><div class="stat-card"><div class="stat-icon stat-icon-orange"><i class="bi bi-x-circle-fill"></i></div><div class="stat-content"><div class="stat-title">Ruangan Nonaktif</div><div class="stat-val"><?= $stats['nonaktif'] ?? 0 ?> Ruangan</div><div class="stat-subtitle">Dinonaktifkan sementara</div></div></div></div></div>
<div class="stat-card-item"><div class="card-3d"><div class="stat-card"><div class="stat-icon stat-icon-red"><i class="bi bi-trash-fill"></i></div><div class="stat-content"><div class="stat-title">Ruangan Terhapus</div><div class="stat-val"><?= $stats['terhapus'] ?? 0 ?> Ruangan</div><div class="stat-subtitle">Bisa dikembalikan</div></div></div></div></div>
...
<a href="list.php<?= !empty($cari) ? '?cari=' . urlencode($cari) . '&' : '?' ?>sort=<?= $sort ?>" class="status-tab <?= $filter_terhapus == 0 && $status_filter === '' ? 'active' : '' ?>"><i class="bi bi-grid"></i> Semua<span class="tab-count"><?= ($stats['aktif'] ?? 0) + ($stats['nonaktif'] ?? 0) ?></span></a>
<a href="list.php?status=1<?= !empty($cari) ? '&cari=' . urlencode($cari) : '' ?>&sort=<?= $sort ?>" class="status-tab <?= $filter_terhapus == 0 && $status_filter === '1' ? 'active' : '' ?>"><i class="bi bi-check-circle"></i> Aktif<span class="tab-count"><?= $stats['aktif'] ?? 0 ?></span></a>
<a href="list.php?status=0<?= !empty($cari) ? '&cari=' . urlencode($cari) : '' ?>&sort=<?= $sort ?>" class="status-tab <?= $filter_terhapus == 0 && $status_filter === '0' ? 'active' : '' ?>"><i class="bi bi-x-circle"></i> Nonaktif<span class="tab-count"><?= $stats['nonaktif'] ?? 0 ?></span></a>
...
<?php if ($filter_terhapus == 0): ?><a href="add.php" class="btn-reg-header text-decoration-none"><i class="bi bi-plus-circle-fill me-2"></i>Tambah Ruangan</a><?php endif; ?>
</div>
...
<td><?php if ($is_deleted): ?><span class="badge-status badge-terhapus"><span class="badge-dot"></span>Terhapus</span><?php elseif ($status == 1): ?><span class="badge-status badge-aktif"><span class="badge-dot"></span>Aktif</span><?php else: ?><span class="badge-status badge-nonaktif"><span class="badge-dot"></span>Nonaktif</span><?php endif; ?></td>
<td>
<?php if (!$is_deleted): ?>
<button class="btn-action-circle btn-action-detail" onclick="bukaDetailModal(<?= $row['ID_Ruangan'] ?>)" title="Lihat Detail"><i class="bi bi-eye"></i></button>
<a href="edit.php?id=<?= $row['ID_Ruangan'] ?>" class="btn-action-circle btn-action-edit" title="Edit Ruangan"><i class="bi bi-pencil"></i></a>
<?php if ($status == 1): ?><button class="btn-action-circle btn-action-toggle" onclick="toggleStatus(<?= $row['ID_Ruangan'] ?>, 1, '<?= htmlspecialchars($row['Nama_Ruangan']) ?>')" title="Nonaktifkan"><i class="bi bi-toggle-on"></i></button><?php else: ?><button class="btn-action-circle btn-action-toggle-off" onclick="toggleStatus(<?= $row['ID_Ruangan'] ?>, 0, '<?= htmlspecialchars($row['Nama_Ruangan']) ?>')" title="Aktifkan"><i class="bi bi-toggle-off"></i></button><?php endif; ?>
<button class="btn-action-circle btn-action-soft-delete" onclick="softDeleteConfirm(<?= $row['ID_Ruangan'] ?>, '<?= htmlspecialchars($row['Nama_Ruangan']) ?>')" title="Hapus"><i class="bi bi-trash"></i></button>
<?php else: ?>
<button class="btn-action-circle btn-action-restore" onclick="restoreConfirm(<?= $row['ID_Ruangan'] ?>, '<?= htmlspecialchars($row['Nama_Ruangan']) ?>')" title="Kembalikan"><i class="bi bi-arrow-counterclockwise"></i></button>
<button class="btn-action-circle btn-action-hard-delete" onclick="hardDeleteConfirm(<?= $row['ID_Ruangan'] ?>, '<?= htmlspecialchars($row['Nama_Ruangan']) ?>')" title="Hapus Permanen"><i class="bi bi-trash-fill"></i></button>
<?php endif; ?>
</td>
</tr>
<?php endforeach; else: ?>
...
<?php if ($halaman > 1): ?><a class="page-link-pag" href="list.php?halaman=<?= $halaman - 1 ?>&cari=<?= urlencode($cari) ?>&status=<?= $status_filter ?>&sort=<?= $sort ?>&terhapus=<?= $filter_terhapus ?>" title="Sebelumnya"><i class="bi bi-chevron-left"></i></a><?php else: ?><span class="page-link-pag disabled"><i class="bi bi-chevron-left"></i></span><?php endif; ?>
<?php $start_page = max(1, $halaman - 2); $end_page = min($total_halaman, $halaman + 2); if ($start_page > 1) { echo '<a class="page-link-pag" href="list.php?halaman=1&cari=' . urlencode($cari) . '&status=' . $status_filter . '&sort=' . $sort . '&terhapus=' . $filter_terhapus . '">1</a>'; if ($start_page > 2) echo '<span class="page-link-pag disabled">...</span>'; } for ($i = $start_page; $i <= $end_page; $i++): ?><a class="page-link-pag <?= ($halaman == $i) ? 'active-pag' : '' ?>" href="list.php?halaman=<?= $i ?>&cari=<?= urlencode($cari) ?>&status=<?= $status_filter ?>&sort=<?= $sort ?>&terhapus=<?= $filter_terhapus ?>"><?= $i ?></a><?php endfor; if ($end_page < $total_halaman) { if ($end_page < $total_halaman - 1) echo '<span class="page-link-pag disabled">...</span>'; echo '<a class="page-link-pag" href="list.php?halaman=' . $total_halaman . '&cari=' . urlencode($cari) . '&status=' . $status_filter . '&sort=' . $sort . '&terhapus=' . $filter_terhapus . '">' . $total_halaman . '</a>'; } ?>
<?php if ($halaman < $total_halaman): ?><a class="page-link-pag" href="list.php?halaman=<?= $halaman + 1 ?>&cari=<?= urlencode($cari) ?>&status=<?= $status_filter ?>&sort=<?= $sort ?>&terhapus=<?= $filter_terhapus ?>" title="Selanjutnya"><i class="bi bi-chevron-right"></i></a><?php else: ?><span class="page-link-pag disabled"><i class="bi bi-chevron-right"></i></span><?php endif; ?>
</nav>
</div>
...
<!-- MODAL DETAIL -->
<div class="modal fade modal-detail" id="modalDetailRuangan" tabindex="-1" aria-hidden="true">
<div class="modal-dialog modal-dialog-centered modal-lg">
<div class="modal-content">
<div class="modal-header"><h5 class="modal-title"><i class="bi bi-door-open-fill me-2"></i>Detail Ruangan</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button></div>
<div class="modal-body" id="detailRuanganContent">
<div class="text-center py-5"><div class="spinner-border text-danger" role="status"><span class="visually-hidden">Loading...</span></div><p class="mt-3 text-muted">Memuat detail ruangan...</p></div>
</div>
</div>
</div>
</div>
...
<?php if ($filter_terhapus == 0): ?><div><label style="display:block;font-size:0.75rem;font-weight:800;color:var(--text-dark);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px">STATUS</label><select class="form-select" id="modalStatus" style="border:2px solid #e2e8f0;border-radius:14px;padding:14px 18px;font-weight:600"><option value="" <?= $status_filter === '' ? 'selected' : '' ?>>Semua Status</option><option value="1" <?= $status_filter === '1' ? 'selected' : '' ?>>Aktif</option><option value="0" <?= $status_filter === '0' ? 'selected' : '' ?>>Nonaktif</option></select></div><?php endif; ?>
</div>
...
function applyFilter(){document.getElementById('hiddenSort').value=document.getElementById('modalSort').value;<?php if($filter_terhapus==0):?>document.getElementById('hiddenStatus').value=document.getElementById('modalStatus').value;<?php endif;?>document.getElementById('mainSearchForm').submit()}
function resetFilter(){document.getElementById('modalSort').value='nama_asc';<?php if($filter_terhapus==0):?>document.getElementById('modalStatus').value='';document.getElementById('hiddenStatus').value='';<?php endif;?>document.getElementById('hiddenSort').value='nama_asc';document.getElementById('mainSearchForm').submit()}
function toggleStatus(id,currentStatus,nama){const actionText=currentStatus===1?'menonaktifkan':'mengaktifkan';const icon=currentStatus===1?'warning':'question';const confirmText=currentStatus===1?'Ya, Nonaktifkan':'Ya, Aktifkan';Swal.fire({title:'Ubah Status Ruangan?',text:'Anda akan '+actionText+' ruangan "'+nama+'"',icon:icon,showCancelButton:true,confirmButtonColor:'#D53D66',cancelButtonColor:'#718096',confirmButtonText:confirmText,cancelButtonText:'Batal'}).then((result)=>{if(result.isConfirmed)window.location.href='action_ruangan.php?aksi=toggle_status&id='+id})}
function softDeleteConfirm(id,nama){Swal.fire({title:'Hapus Ruangan?',html:'Ruangan <b>"'+nama+'"</b> akan dihapus.<br><br><span style="color:#059669"><i class="bi bi-info-circle-fill"></i> Data bisa dikembalikan nanti.</span>',icon:'warning',showCancelButton:true,confirmButtonColor:'#dc2626',cancelButtonColor:'#718096',confirmButtonText:'<i class="bi bi-trash"></i> Ya, Hapus',cancelButtonText:'Batal'}).then((result)=>{if(result.isConfirmed)window.location.href='action_ruangan.php?aksi=soft_delete&id='+id})}
function restoreConfirm(id,nama){Swal.fire({title:'Kembalikan Ruangan?',html:'Ruangan <b>"'+nama+'"</b> akan dikembalikan ke daftar aktif.',icon:'question',showCancelButton:true,confirmButtonColor:'#059669',cancelButtonColor:'#718096',confirmButtonText:'<i class="bi bi-arrow-counterclockwise"></i> Ya, Kembalikan',cancelButtonText:'Batal'}).then((result)=>{if(result.isConfirmed)window.location.href='action_ruangan.php?aksi=restore&id='+id})}
function hardDeleteConfirm(id,nama){Swal.fire({title:'HAPUS PERMANEN?',html:'Ruangan <b>"'+nama+'"</b> akan dihapus <span style="color:#dc2626;font-weight:800">PERMANEN</span> dari database!<br><br><i class="bi bi-exclamation-triangle-fill" style="color:#dc2626"></i> Data tidak bisa dikembalikan!',icon:'error',showCancelButton:true,confirmButtonColor:'#7c2d12',cancelButtonColor:'#718096',confirmButtonText:'<i class="bi bi-trash-fill"></i> Ya, Hapus Permanen',cancelButtonText:'Batal'}).then((result)=>{if(result.isConfirmed)window.location.href='action_ruangan.php?aksi=hard_delete&id='+id})}
var detailModal;
function bukaDetailModal(id){detailModal=new bootstrap.Modal(document.getElementById('modalDetailRuangan'));detailModal.show();fetch('ajax_detail_ruangan.php?id='+id).then(response=>response.text()).then(html=>{document.getElementById('detailRuanganContent').innerHTML=html}).catch(()=>{document.getElementById('detailRuanganContent').innerHTML='<div class="alert alert-danger">Gagal memuat detail ruangan.</div>'})}
