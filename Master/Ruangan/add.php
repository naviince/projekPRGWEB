        ...
        if (sqlsrv_begin_transaction($conn) === false) {
            ...
            try {
                $sql_ruangan = "INSERT INTO Ruangan 
                    (Nama_Ruangan, Deskripsi, Foto_Ruangan, Status, Is_Deleted, Created_By, Created_Date) 
                    OUTPUT INSERTED.ID_Ruangan
                    VALUES (?, ?, ?, ?, 0, ?, GETDATE())";

                $params_ruangan = [$nama, $deskripsi, $new_filename, $status, $nama_admin];
...
<li><a href="../../Transaksi/Penjualan/list.php" class="submenu-link"><i class="bi bi-bag-fill me-2"></i>Penjualan Barang Cetak</a></li>
                                ...
                                <div class="paket-grid">
                                    ...
                                    ?>
                                        <div class="paket-checkbox-item <?= $is_selected ?>" onclick="togglePaket(this)">
                                            <input type="checkbox" name="paket[]" value="<?= $paket['ID_Paket'] ?>" <?= $is_checked ?> onchange="updatePaketCount()">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="status-toggle-group">
                            <label class="status-option active" onclick="selectStatus(this, 1)">
                                ...
                                <div class="status-icon">✅</div><div class="status-label">Aktif</div><div class="status-desc">Bisa dipesan</div>
                            </label>
                            <label class="status-option" onclick="selectStatus(this, 0)">
                                <input type="radio" name="status" value="0">
                                <div class="status-icon">⛔</div><div class="status-label">Nonaktif</div><div class="status-desc">Sementara tutup</div>
                            </label>
                        </div>
                    </div>

                    <div class="d-flex gap-3 mt-4">
                        <button type="submit" name="simpan" class="btn-submit" id="btnSimpan"><i class="bi bi-check2-circle"></i> Simpan Ruangan</button>
    ...
    <script>
        ...
        <?php if ($success): ?> Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Ruangan studio baru telah ditambahkan dan terhubung ke paket foto.', confirmButtonColor: '#D53D66' }).then(() => { window.location.href = 'list.php?status_sukses=tambah'; }); <?php endif; ?>
    </script>
</body>
</html>