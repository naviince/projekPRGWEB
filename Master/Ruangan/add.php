    ...
    if ($kapasitas <= 0 || $harga < 0) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            if (move_uploaded_file($foto_tmp, $upload_path)) {
                $sql = "INSERT INTO Ruangan (Nama_Ruangan, Kapasitas_Ruangan, Harga_Sewa, Luas_Ruangan, Fasilitas, Deskripsi, Foto_Ruangan, Status_Ruangan, Created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, 'Aktif', GETDATE())";
                $params = array($nama, $kapasitas, $harga, $luas, $fasilitas, $desc, $new_filename);
...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .side-visual { width:38%; flex-shrink:0; background:linear-gradient(150deg,var(--pink-deep) 0%,var(--pink) 100%); padding:50px 40px; color:white; display:flex; flex-direction:column; justify-content:space-between; position:relative; overflow:hidden; }
        ...
        .form-control { background:#f8fafc!important; border:2px solid transparent!important; border-radius:12px!important; padding:12px 15px!important; font-size:14px!important; transition:border-color 0.25s,box-shadow 0.25s!important; }
        ...
        #preview-img { display:none; width:100%; max-height:190px; object-fit:cover; border-radius:12px; margin-top:12px; border:2px solid #ffe0ec; }
        .btn-simpan { background:linear-gradient(135deg,var(--pink),var(--pink-dark)); color:white; border-radius:14px; padding:14px 28px; font-weight:800; border:none; width:100%; font-size:15px; transition:transform 0.25s,box-shadow 0.25s; margin-top:8px; }
        .btn-simpan:hover { transform:translateY(-3px); box-shadow:0 12px 28px rgba(232,69,122,0.35); color:white; }
    ...
    <div class="main-card">
        ...
        <div class="side-visual">
            <div>
                ...
                <p>Tambahkan ruangan studio beserta harga sewa dan fasilitasnya agar tersedia untuk booking pelanggan.</p>
            </div>
            <div class="tips-box">
                <i class="bi bi-lightbulb-fill me-1"></i>
                <b>Tips:</b> Isi harga sewa, kapasitas, dan fasilitas secara detail agar pelanggan dapat memilih ruangan yang sesuai kebutuhan.
            </div>
        </div>
        <!-- Sisi Kanan -->
        <div class="form-side">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="harga_sewa" class="form-control" required placeholder="200000" min="0">
                        </div>
                    </div>
                </div>
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="fasilitas" class="form-control" placeholder="AC, Backdrop, Lighting">
                    </div>
                </div>
                <div class="mb-3">
                    <label class="form-label">Deskripsi Ruangan</label>
                    <textarea name="deskripsi" class="form-control" rows="3" required placeholder="Deskripsikan suasana dan keunggulan ruangan..."></textarea>
                </div>
                <div class="mb-4">
                ...
                <button type="submit" name="simpan" class="btn btn-simpan"><i class="bi bi-check2-circle me-2"></i>Simpan Ruangan</button>
...
<?php if ($success): ?>
<script>Swal.fire({icon:'success',title:'Berhasil!',text:'Ruangan studio baru telah ditambahkan.',confirmButtonColor:'#e8457a'}).then(()=>window.location='list.php');</script>
<?php endif; ?>
<script>
