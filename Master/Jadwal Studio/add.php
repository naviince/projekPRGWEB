...
function safe_sqlsrv_fetch($conn, $sql, $params = []) {
    ...
    if ($stmt === false) { error_log("[safe_sqlsrv_fetch] SQL Error: " . json_encode(sqlsrv_errors(SQLSRV_ERR_ALL))); return null; }
...
function safe_sqlsrv_fetch_all($conn, $sql, $params = []) {
    $stmt = sqlsrv_query($conn, $sql, $params);
    if ($stmt === false) { error_log("[safe_sqlsrv_fetch_all] SQL Error: " . json_encode(sqlsrv_errors(SQLSRV_ERR_ALL))); return []; }
    $results = [];
...
function safe_sqlsrv_count($conn, $sql, $params = []) {
    $stmt = sqlsrv_query($conn, $sql, $params);
    if ($stmt === false) { error_log("[safe_sqlsrv_count] SQL Error: " . json_encode(sqlsrv_errors(SQLSRV_ERR_ALL))); return 0; }
...
if (isset($_POST['simpan'])) {
    ...
    } elseif (!isset($paket_ruangan_map[$id_paket]) || empty($paket_ruangan_map[$id_paket])) {
        $errors['id_paket'] = "Paket Foto ini tidak memiliki ruangan yang terhubung. Hubungi admin untuk mengatur Paket-Ruangan.";
    } else {
    ...
    if (empty($errors) && $id_ruangan > 0 && !empty($tanggal_jadwal) && !empty($jam_selesai)) {
        $cek_overlap = safe_sqlsrv_count($conn,
            "SELECT COUNT(*) as total FROM Jadwal_Studio WHERE ID_Ruangan = ? AND Tanggal_Jadwal = ? AND Is_Deleted = 0 AND Status = 1 AND ((Jam_Mulai < ? AND Jam_Selesai > ?) OR (Jam_Mulai >= ? AND Jam_Mulai < ?) OR (Jam_Selesai > ? AND Jam_Selesai <= ?))",
            [$id_ruangan, $tanggal_jadwal, $jam_selesai . ':00', $jam_mulai . ':00', $jam_mulai . ':00', $jam_selesai . ':00', $jam_mulai . ':00', $jam_selesai . ':00']
        );
        if ($cek_overlap > 0) $errors['jam_mulai'] = "Jadwal bertabrakan dengan {$cek_overlap} slot existing. Pilih jam atau tanggal lain!";
    }

    // =====================================================
    // INSERT DATA — SUDAH DIPERBAIKI (MEMASUKKAN ID_PAKET)
    // =====================================================
    if (empty($errors)) {
        if (empty($keterangan)) $keterangan = "Slot {$nama_paket} {$nama_ruangan}";

        // ID_Paket kini ditambahkan ke dalam query insert
        $sql = "INSERT INTO Jadwal_Studio (ID_Ruangan, ID_Paket, Tanggal_Jadwal, Jam_Mulai, Jam_Selesai, Keterangan, Status_Jadwal, Status, Created_By, Created_Date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, GETDATE())";
        $params = [$id_ruangan, $id_paket, $tanggal_jadwal, $jam_mulai . ':00', $jam_selesai . ':00', $keterangan, $status_jadwal, STATUS_DATA_AKTIF, $nama_admin];
        ...
        if ($stmt) {
            sqlsrv_free_stmt($stmt);
            $success = true;
            error_log("[Jadwal Insert] SUCCESS — ID_Ruangan: {$id_ruangan}, ID_Paket: {$id_paket}, Tanggal: {$tanggal_jadwal}, Jam: {$jam_mulai}-{$jam_selesai}");
        } else {
            $sql_errors = sqlsrv_errors(SQLSRV_ERR_ALL);
            $error_msg = "Gagal menyimpan jadwal. ";
            if ($sql_errors && is_array($sql_errors) && count($sql_errors) > 0) {
                foreach ($sql_errors as $error) {
                    $error_msg .= "[SQLSTATE: {$error['SQLSTATE']}, Code: {$error['code']}] {$error['message']}. ";
                }
            } else {
                $conn_errors = sqlsrv_errors(SQLSRV_ERR_ERRORS);
                if ($conn_errors && is_array($conn_errors)) {
                    foreach ($conn_errors as $err) {
                        $error_msg .= "[CONN] {$err['message']}. ";
                    }
                } else {
                    $error_msg .= "Tidak ada detail error dari database. Cek log PHP atau SQL Server.";
                }
            }
            $errors['general'] = $error_msg;
            error_log("[Jadwal Insert] FAILED: " . $error_msg);
        }
    }
}
...
@media(max-width:992px){.main-content{margin-left:0;padding:20px}.sidebar{transform:translateX(-100%)}.form-card{padding:25px}.btn-group-bottom{flex-direction:column}.btn-simpan,.btn-kembali{width:100%;justify-content:center}.paket-grid{grid-template-columns:repeat(2,1fr)}.ruangan-grid{grid-template-columns:repeat(2,1fr)}.form-grid{grid-template-columns:1fr}}
...
<div><h3 class="fw-bold mb-1">Tambah Jadwal Studio</h3><p class="text-muted small mb-0">Buat slot jadwal baru dengan validasi paket dan ruangan.</p></div>
...
<div class="info-card"><i class="bi bi-info-circle-fill"></i><div class="info-text"><strong>Perhatian:</strong> Pilih <strong>Paket Foto</strong> terlebih dahulu. Ruangan akan disesuaikan otomatis dengan paket yang dipilih. Jam operasional: <strong>08:00 - 20:00</strong>.</div></div>
...
<?php foreach ($paket_list as $paket): $is_selected = (isset($_POST['id_paket']) && $_POST['id_paket'] == $paket['ID_Paket']); ?>
...
<?php foreach ($ruangan_list as $ruangan): $is_selected = (isset($_POST['id_ruangan']) && $_POST['id_ruangan'] == $ruangan['ID_Ruangan']); $current_paket_id = isset($_POST['id_paket']) ? (int)$_POST['id_paket'] : 0; $valid_for_paket = true; if ($current_paket_id > 0) $valid_for_paket = in_array((int)$ruangan['ID_Ruangan'], $paket_ruangan_map[$current_paket_id] ?? []); ?>
...
<div class="form-grid mb-3">
<div><label class="form-label-custom">Status Jadwal</label><select name="status_jadwal" class="form-input-custom"><option value="<?= STATUS_JADWAL_TERSEDIA ?>" <?= (!isset($old_values['status_jadwal']) || $old_values['status_jadwal'] == STATUS_JADWAL_TERSEDIA) ? 'selected' : '' ?>>Tersedia</option><option value="<?= STATUS_JADWAL_MAINTENANCE ?>" <?= (isset($old_values['status_jadwal']) && $old_values['status_jadwal'] == STATUS_JADWAL_MAINTENANCE) ? 'selected' : '' ?>>Maintenance</option></select><div class="helper-text"><i class="bi bi-info-circle"></i>Status "Booked" akan otomatis saat ada order.</div></div>
<div><label class="form-label-custom">Keterangan <span style="color:#94a3b8;font-weight:500">(opsional)</span></label><input type="text" name="keterangan" class="form-input-custom" value="<?= htmlspecialchars($old_values['keterangan'] ?? '') ?>" placeholder="Contoh: Slot Basic Studio A"><div class="helper-text"><i class="bi bi-magic"></i>Akan di-generate otomatis: "Slot [Paket] [Ruangan]"</div></div>
</div>
<div class="btn-group-bottom"><a href="list.php" class="btn-kembali"><i class="bi bi-arrow-left"></i>Kembali</a><button type="submit" name="simpan" class="btn-simpan"><i class="bi bi-check-circle-fill"></i>Simpan Jadwal</button></div>
</form>
</div>
</div>
...
function selectPaket(card,id,durasi,nama,harga){
    ...
    // Perbaikan UX: Reset pilihan ruangan secara visual saat paket diganti
...
<?php if($success): ?><script>Swal.fire({icon:'success',title:'Berhasil!',text:'Jadwal studio baru berhasil ditambahkan.',confirmButtonColor:'#D53D66',confirmButtonText:'Oke'}).then(()=>{window.location.href='list.php?status_sukses=tambah'})</script><?php endif; ?>
</body>
</html>
<?php ob_end_flush(); ?>