...
if (isset($_POST['simpan'])) {
    ...
    if (empty($errors)) {
        ...
        if ($stmt) {
            ...
            if ($id_jadwal_baru) {
                // Update Status_Jadwal ke 0 (Tersedia) agar langsung aktif untuk dibooking pelanggan
            ...
            if (!empty($sql_errors)) {
                // Menangkap pesan RAISERROR asli dari database jika ada bentrok jadwal
...
<div class="main-content">
    <div class="dashboard-header fade-in-up">
        <div><h3 class="fw-bold mb-1">Tambah Jadwal Studio</h3><p class="text-muted small mb-0">Buat slot jadwal pemotretan baru.</p></div>
    ...
    <!-- UI DESIGN SINKRON SESUAI STRUKTUR KODE PROGRAM AWAL ANDA -->
    <div class="form-card fade-in-up">
        ...
        <!-- Tampilan Pesan Error Sinkron Sesuai Desain Awal Anda -->
        ...
        <form method="POST" id="formJadwal">
            <h4 class="section-title"><i class="bi bi-calendar-event-fill"></i> Form Jadwal Baru</h4>
            <p class="text-muted small mb-4">Lengkapi informasi jadwal, tentukan paket foto, dan ruangan terkait.</p>

            <div class="form-grid mb-4">
                <div>
                    <label class="form-label-custom">Paket Foto <span class="text-danger">*</span></label>
                    <select name="id_paket" id="idPaket" class="form-input-custom <?= isset($errors['id_paket']) ? 'is-invalid' : '' ?>" required>
                    ...
                    <div class="helper-text"><i class="bi bi-info-circle"></i>Pilih paket foto terlebih dahulu.</div>
                </div>
                <div>
                    <label class="form-label-custom">Ruangan <span class="text-danger">*</span></label>
                    <select name="id_ruangan" id="idRuangan" class="form-input-custom <?= isset($errors['id_ruangan']) ? 'is-invalid' : '' ?>" required>
                    ...
                    <div class="helper-text"><i class="bi bi-info-circle"></i>Ruangan akan muncul sesuai paket yang dipilih.</div>
                </div>
            </div>

            <div class="durasi-preview mb-4" id="durasiPreview"><i class="bi bi-clock-history"></i><div class="durasi-text">Durasi Paket: <strong id="durasiText">-</strong> menit<br><span style="font-size: 0.8rem; color: #718096;">Jam selesai dihitung otomatis</span></div></div>

            <div class="form-grid mb-4">
                <div>
                    <label class="form-label-custom">Tanggal Jadwal <span class="text-danger">*</span></label>
                    <input type="date" name="tanggal_jadwal" id="tanggalJadwal" class="form-input-custom <?= isset($errors['tanggal_jadwal']) ? 'is-invalid' : '' ?>" value="<?= htmlspecialchars($old_values['tanggal_jadwal'] ?? '') ?>" min="<?= date('Y-m-d') ?>" required>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="keterangan" class="form-input-custom <?= isset($errors['keterangan']) ? 'is-invalid' : '' ?>" value="<?= htmlspecialchars($old_values['keterangan'] ?? '') ?>" placeholder="Contoh: Slot Basic Studio A, Libur Hari Raya, Maintenance..." maxlength="255">
...
// Jalankan saat halaman selesai dimuat untuk menampilkan preview durasi jika paket sudah terpilih (pasca-reload error)
