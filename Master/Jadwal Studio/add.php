...
if (isset($_POST['simpan'])) {
    ...
    if (empty($id_ruangan) || empty($tanggal) || empty($jam_mulai) || empty($jam_selesai)) {
        ...
        $error = "Jam mulai tidak boleh lebih besar atau sama dengan jam selesai!";
    } else {
        ...
        $params_cek = [$id_ruangan, $tanggal, $jam_selesai, $jam_mulai, $jam_mulai, $jam_selesai];
        $res_cek = safe_sqlsrv_fetch($conn, $sql_cek, $params_cek);

        if (($res_cek['total'] ?? 0) > 0) {
            $error = "Gagal! Ruangan tersebut sudah memiliki jadwal di jam yang bersinggungan.";
        } else {
            $sql_ins = "INSERT INTO Jadwal_Studio (ID_Ruangan, Tanggal_Jadwal, Jam_Mulai, Jam_Selesai, Keterangan, Status_Jadwal, Status, Is_Deleted, Created_By, Created_Date) 
                        VALUES (?, ?, ?, ?, ?, 0, ?, 0, ?, GETDATE())";
            ...
            else { $error = "Terjadi kesalahan database saat menyimpan data."; }
        }
    }
}
?>
...
<head>
    ...
    <style>
        ...
        .btn-logoutt:hover { transform: translateY(-2px); box-shadow: 0 6px 15px rgba(213, 61, 102, 0.2); }
        ...
        .btn-logout { background: #D53D66 !important; color: white !important; border: none; width: 100%; padding: 12px; border-radius: 50px; font-weight: 800; font-size: 1rem; transition: var(--transition-3d); display: flex; align-items: center; justify-content: center; gap: 10px; box-shadow: 0 4px 12px rgba(213, 61, 102, 0.2); }
...
<!-- SIDEBAR -->
    <div class="sidebar">
        ...
        <div>
            <button onclick="confirmLogout(event)" class="btn btn-logoutt text-center d-block w-100">
...
<div class="main-content">
    <div class="dashboard-header">
        <div><h3 class="fw-bold mb-1">Tambah Jadwal Studio</h3><p class="text-muted small mb-0">Buat slot waktu operasional baru untuk studio foto.</p></div>
    ...
    <div class="form-card fade-in-up">
        <div class="form-card-header">
            ...
            <p>Tentukan ruangan, tanggal, dan jam operasional slot foto.</p>
        </div>
            ...
            <form method="POST">
                <div class="row">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="date" name="tanggal_jadwal" class="form-control-custom" required min="<?= date('Y-m-d') ?>">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="time" name="jam_mulai" class="form-control-custom" required>
                    </div>
                    <div class="col-md-6 mb-4">
                        <label class="form-label">Jam Selesai <span>*</span></label>
                        <input type="time" name="jam_selesai" class="form-control-custom" required>
                    </div>
                </div>

                <div class="mb-4">
                    <label class="form-label">Keterangan / Memo</label>
                    <textarea name="keterangan" class="form-control-custom" rows="3" placeholder="Contoh: Slot khusus pagi atau promo (opsional)"></textarea>
                </div>
                ...
                <div class="d-flex gap-3 mt-5">
                    <button type="submit" name="simpan" class="btn-submit"><i class="bi bi-check2-circle"></i> Simpan Jadwal</button>
...
<script>
    ...
    function updateClock() {
        const now = new Date();
        document.getElementById('live-clock').innerText = now.toLocaleString('id-ID', { dateStyle: 'full', timeStyle: 'medium' }) + " WIB";
    }
...
<?php if($success): ?>
<script>
    Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Jadwal studio baru telah ditambahkan.', confirmButtonColor: '#D53D66' })
