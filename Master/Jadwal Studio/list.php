...
$stats = safe_sqlsrv_fetch($conn, 
    "SELECT 
        COUNT(*) as total,
        SUM(CASE WHEN Status_Jadwal = 0 AND Status = 1 THEN 1 ELSE 0 END) as tersedia,
        SUM(CASE WHEN Status_Jadwal = 1 THEN 1 ELSE 0 END) as terpesan,
        SUM(CASE WHEN Status_Jadwal = 2 THEN 1 ELSE 0 END) as selesai
    FROM Jadwal_Studio WHERE Is_Deleted = 0"
) ?? ['total' => 0, 'tersedia' => 0, 'terpesan' => 0, 'selesai' => 0];
...
<body>
        ...
        <div class="stats-scroll-wrapper animate-fade-in">
            <div class="stats-row">
                <div class="stat-card-item">
                    <div class="card-3d">
                        <div class="stat-card">
                            <div class="stat-icon stat-icon-pink"><i class="bi bi-calendar3"></i></div>
                            <div class="stat-content">
                                <div class="stat-title">Total Jadwal</div>
                                <div class="stat-val"><?= $stats['total'] ?? 0 ?> Jadwal</div>
                                <div class="stat-subtitle">Semua slot waktu</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-3d">
                        <div class="stat-card">
                            ...
                            <div class="stat-content">
                                ...
                                <div class="stat-val"><?= $stats['tersedia'] ?? 0 ?> Jadwal</div>
                                <div class="stat-subtitle">Siap di-booking</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="stat-card-item">
                    <div class="card-3d">
                        <div class="stat-card">
                            <div class="stat-icon stat-icon-orange"><i class="bi bi-bookmark-check-fill"></i></div>
                            <div class="stat-content">
                                <div class="stat-title">Terpesan</div>
                                <div class="stat-val"><?= $stats['terpesan'] ?? 0 ?> Jadwal</div>
                                <div class="stat-subtitle">Sedang aktif</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="stat-card-item">
                    <div class="card-3d">
                        <div class="stat-card">
                            <div class="stat-icon stat-icon-blue"><i class="bi bi-check-all"></i></div>
                            <div class="stat-content">
                                <div class="stat-title">Selesai</div>
                                <div class="stat-val"><?= $stats['selesai'] ?? 0 ?> Jadwal</div>
                                <div class="stat-subtitle">Sesi foto selesai</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="search-filter-bar">
            <form method="GET" class="search-form-flex" id="mainSearchForm">
                ...
                <div class="search-input-wrapper">
                    ...
                    <input type="text" name="cari" class="search-input-main" placeholder="Cari ruangan atau keterangan..." value="<?= htmlspecialchars($cari) ?>">
                </div>
        ...
        <div class="card-3d mb-4" style="padding: 24px;">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        if (!empty($daftar_jadwal)):
                            foreach($daftar_jadwal as $idx => $j):
                                // Status badge
                                $badge_class = '';
                                $status_text = '';
                                $status_icon = '';
                                if ($j['Status_Jadwal'] == 0) {
                                    $badge_class = 'badge-tersedia';
                                    $status_text = 'Tersedia';
                                    $status_icon = 'bi-check-circle';
                                } elseif ($j['Status_Jadwal'] == 1) {
                                    $badge_class = 'badge-terpesan';
                                    $status_text = 'Terpesan';
                                    $status_icon = 'bi-bookmark-check';
                                } else {
                            ...
                            <tr class="fade-in-up">
                                ...
                                <td>
                                    <span class="badge-ruangan">
                                        ...
                                        <?= htmlspecialchars($j['Nama_Ruangan'] ?? 'Ruangan Tidak Ditemukan') ?>
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <strong><?= formatWaktu($j['Jam_Mulai']) ?></strong> - <strong><?= formatWaktu($j['Jam_Selesai']) ?></strong>
                                </td>
                                ...
                                <td>
                                    <a href="edit.php?id=<?= $j['ID_Jadwal'] ?>" class="btn-action-circle btn-action-edit" title="Edit Jadwal">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <button class="btn-action-circle btn-action-delete" onclick="toggleStatus(<?= $j['ID_Jadwal'] ?>, <?= $j['Status'] ?>, '<?= htmlspecialchars($j['Nama_Ruangan'] ?? '') ?>')" title="Toggle Status">
                                        <i class="bi bi-toggle-<?= $j['Status'] == 1 ? 'on' : 'off' ?>"></i>
                                    </button>
                                    <button class="btn-action-circle btn-action-delete" onclick="softDelete(<?= $j['ID_Jadwal'] ?>, '<?= htmlspecialchars($j['Nama_Ruangan'] ?? '') ?>')" title="Hapus Jadwal">
                        ...
                        ?>
                            <tr>
                                <td colspan="7" class="text-center text-muted py-5">
                                    ...
                                    <p class="small">Coba ubah filter atau tambah jadwal baru.</p>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Menampilkan <span><?= $offset + 1 ?></span> - <span><?= min($offset + $limit, $total_records) ?></span> dari <span><?= $total_records ?></span> jadwal studio
                </div>
                <nav class="pagination-nav">
                    <?php 
                    $base_qs = "cari=" . urlencode($cari) . "&status=" . $status_filter . "&ruangan=" . urlencode($ruangan_filter) . "&sort=" . $sort;
                    ?>
                    <?php if ($halaman > 1): ?>
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Menampilkan <span>1</span> - <span><?= $total_records ?></span> dari <span><?= $total_records ?></span> jadwal studio
                </div>
            </div>
            <?php endif; ?>
        </div>
                ...
                <div class="modal-body" style="padding: 0 24px 20px; background: #ffffff;">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="modalSort" style="border: 2px solid #e2e8f0; border-radius: 14px; padding: 14px 18px; font-weight: 600;">
                            <option value="tanggal_desc" <?= $sort == 'tanggal_desc' ? 'selected' : '' ?>>Tanggal Terbaru</option>
                            <option value="tanggal_asc" <?= $sort == 'tanggal_asc' ? 'selected' : '' ?>>Tanggal Terlama</option>
                            ...
                            <option value="waktu_asc" <?= $sort == 'waktu_asc' ? 'selected' : '' ?>>Waktu Pagi - Malam</option>
                            <option value="waktu_desc" <?= $sort == 'waktu_desc' ? 'selected' : '' ?>>Waktu Malam - Pagi</option>
                        </select>
                    </div>
                    <div class="mb-3">
                    ...
                    <div>
                        <label style="display: block; font-size: 0.75rem; font-weight: 800; color: var(--text-dark); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 10px;">STATUS JADWAL</label>
                        <select class="form-select" id="modalStatus" style="border: 2px solid #e2e8f0; border-radius: 14px; padding: 14px 18px; font-weight: 600;">
                            ...
                            <option value="0" <?= $status_filter === '0' ? 'selected' : '' ?>>Tersedia</option>
                            <option value="1" <?= $status_filter === '1' ? 'selected' : '' ?>>Terpesan</option>
    ...
    <script>
        ...
        function toggleStatus(id, currentStatus, nama) {
            ...
            Swal.fire({
                ...
                text: 'Anda akan ' + actionText + ' jadwal untuk ruangan "' + nama + '"',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    window.location.href = 'action_jadwal.php?aksi=toggle_status&id=' + id;
                }
            });
        }
        ...
        function softDelete(id, nama) {
            Swal.fire({
                title: 'Hapus Jadwal?',
                text: 'Anda akan menghapus jadwal untuk ruangan "' + nama + '" (soft delete)',
                icon: 'warning',
                showCancelButton: true,
    ...
    <script>
        ...
        else if ("<?= $_GET['status_sukses'] ?>" == 'toggle_status') { msg = "<?= $_GET['message'] ?? 'Status jadwal berhasil diubah!' ?>"; t_title = "Status Diubah"; }
