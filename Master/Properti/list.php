...
// =====================================================
// DAFTAR RUANGAN & KATEGORI (UNTUK FILTER)
// =====================================================
$daftar_ruangan_filter = safe_sqlsrv_fetch_all($conn,
    "SELECT ID_Ruangan, Nama_Ruangan FROM Ruangan WHERE Is_Deleted = 0 ORDER BY Nama_Ruangan ASC"
);

$daftar_kategori_filter = safe_sqlsrv_fetch_all($conn,
    "SELECT DISTINCT Kategori_Properti FROM Properti WHERE Is_Deleted = 0 AND Kategori_Properti IS NOT NULL ORDER BY Kategori_Properti ASC"
);
...
if (!empty($cari)) {
    $conditions[] = "(p.Nama_Properti LIKE ? OR p.Deskripsi LIKE ?)";
    $params[] = "%$cari%"; 
    $params[] = "%$cari%";
}
if ($status_filter !== "") {
...
if ($kategori_filter !== "") {
    $conditions[] = "p.Kategori_Properti = ?";
...
elseif ($sort == "kategori_asc") { $order_clause = "p.Kategori_Properti ASC"; }
elseif ($sort == "kategori_desc") { $order_clause = "p.Kategori_Properti DESC"; }

// Hitung total untuk pagination
$count_sql = "SELECT COUNT(*) AS total FROM Properti p 
...
// Ambil data dengan relasi ruangan
$list_sql = "SELECT 
    p.ID_Properti,
    p.ID_Ruangan,
    p.Nama_Properti,
...
<head>
    ...
    <style>
        ...
        .data-table tbody tr:hover .properti-preview { transform: scale(1.08) rotate(2deg); }
        ...
        .td-ruangan { font-size: 0.85rem; color: #4a5568; font-weight: 600; }
...
<li><a href="../../Transaksi/Penjualan/list.php" class="submenu-link"><i class="bi bi-bag-fill me-2"></i>Penjualan Barang Cetak</a></li>
    ...
    <div class="main-content">
        ...
        <div class="stats-scroll-wrapper animate-fade-in">
            <div class="stats-row">
                    ...
                    <div class="card-3d">
                        <div class="stat-card">
                            ...
                            <div class="stat-content">
                                ...
                                <div class="stat-val"><?= $stats['total'] ?? 0 ?> Properti</div>
                    ...
                    <div class="card-3d">
                        <div class="stat-card">
                            ...
                            <div class="stat-content">
                                ...
                                <div class="stat-val"><?= $stats['aktif'] ?? 0 ?> Properti</div>
                ...
                <div class="stat-card-item">
                    <div class="card-3d">
                        <div class="stat-card">
                            ...
                            <div class="stat-content">
                                <div class="stat-title">Properti Nonaktif</div>
                                <div class="stat-val"><?= $stats['nonaktif'] ?? 0 ?> Properti</div>
                ...
                <div class="stat-card-item">
                    <div class="card-3d">
                        <div class="stat-card">
                            <div class="stat-icon stat-icon-blue"><i class="bi bi-tags-fill"></i></div>
                            <div class="stat-content">
                                <div class="stat-title">Kategori Terbanyak</div>
                                <div class="stat-val" style="font-size: 1.1rem;"><?= $top_kategori ? htmlspecialchars($top_kategori['Kategori_Properti']) : '-' ?></div>
                                <div class="stat-subtitle"><?= $top_kategori ? ($top_kategori['total_kategori'] ?? 0) . ' properti' : 'Belum ada data' ?></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="search-filter-bar">
            <form method="GET" class="search-form-flex" id="mainSearchForm">
                ...
                <div class="search-input-wrapper">
                    ...
                    <input type="text" name="cari" class="search-input-main" placeholder="Cari nama properti atau deskripsi..." value="<?= htmlspecialchars($cari) ?>">
                </div>
            ...
            <a href="add.php" class="btn-reg-header text-decoration-none">
                <i class="bi bi-plus-circle-fill me-2"></i>Tambah Properti
            </a>
        </div>
        ...
        <div class="alert alert-light border-2 border-dashed mb-3" style="border-color: #e2e8f0; border-radius: 14px; background: #f8fafc;">
            ...
            <span class="small fw-bold text-muted">
                <strong>Info:</strong> Properti melekat pada satu ruangan dan akan ditampilkan otomatis kepada pelanggan 
                saat ruangan tersebut dipilih. Kelola ruangan di menu <a href="../Ruangan/list.php" style="color: var(--p-pink);">Ruangan</a>.
            </span>
        </div>
        ...
        <div class="card-3d mb-4" style="padding: 24px;">
            <div class="table-scroll-wrapper">
                <table class="data-table">
                    ...
                    <tbody>
                        <?php
                        if (!empty($properti_list)):
                            foreach($properti_list as $row):
                                $path_img = "../../assets/img/properti/" . ($row['Foto_Properti'] ?? '');
                                $img_src = (!empty($row['Foto_Properti']) && file_exists($path_img))
                                    ? $path_img 
                                    : $default_svg_avatar;
                        ...
                        ?>
                            <tr class="fade-in-up">
                                <td>
                                    <div class="d-flex align-items-center gap-3">
                                        <img src="<?= $img_src ?>" class="properti-preview" alt="<?= htmlspecialchars($row['Nama_Properti']) ?>">
                                        <div>
                                            <div class="td-nama"><?= htmlspecialchars($row['Nama_Properti']) ?></div>
                                ...
                                <td>
                                    <span class="badge-kategori">
                                        <i class="bi bi-tag-fill"></i> <?= htmlspecialchars($row['Kategori_Properti'] ?? '-') ?>
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <a href="edit.php?id=<?= $row['ID_Properti'] ?>" class="btn-action-circle btn-action-edit" title="Edit Properti">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <button class="btn-action-circle btn-action-delete" onclick="toggleStatus(<?= $row['ID_Properti'] ?>, <?= $row['Status'] ?>, '<?= htmlspecialchars($row['Nama_Properti']) ?>')" title="Toggle Status">
                                        <i class="bi bi-toggle-<?= $row['Status'] == 1 ? 'on' : 'off' ?>"></i>
                                    </button>
                                    <button class="btn-action-circle btn-action-delete" onclick="hardDelete(<?= $row['ID_Properti'] ?>, '<?= htmlspecialchars($row['Nama_Properti']) ?>')" title="Hapus Permanen">
                        ...
                        ?>
                            <tr>
                                <td colspan="5" class="text-center text-muted py-5">
                                    ...
                                    <p class="fw-bold">Tidak ada data properti yang sesuai.</p>
                                    <p class="small">Coba ubah filter atau tambah properti baru.</p>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Menampilkan <span><?= $offset + 1 ?></span> - <span><?= min($offset + $limit, $total_records) ?></span> dari <span><?= $total_records ?></span> properti
                </div>
                <nav class="pagination-nav">
                    <?php 
                    $base_qs = "cari=" . urlencode($cari) . "&status=" . $status_filter . "&ruangan=" . $ruangan_filter . "&kategori=" . urlencode($kategori_filter) . "&sort=" . $sort;
                    ?>
                    <?php if ($halaman > 1): ?>
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Menampilkan <span>1</span> - <span><?= $total_records ?></span> dari <span><?= $total_records ?></span> properti
                </div>
            </div>
            <?php endif; ?>
        </div>
                ...
                <div class="modal-body" style="padding: 0 24px 20px; background: #ffffff;">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="modalSort" style="border: 2px solid #e2e8f0; border-radius: 14px; padding: 14px 18px; font-weight: 600;">
                            ...
                            <option value="ruangan_desc" <?= $sort == 'ruangan_desc' ? 'selected' : '' ?>>Ruangan Z - A</option>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="modalRuangan" style="border: 2px solid #e2e8f0; border-radius: 14px; padding: 14px 18px; font-weight: 600;">
                            <option value="" <?= $ruangan_filter === '' ? 'selected' : '' ?>>Semua Ruangan</option>
                            <?php foreach ($daftar_ruangan_filter as $r): ?>
                                <option value="<?= $r['ID_Ruangan'] ?>" <?= $ruangan_filter == $r['ID_Ruangan'] ? 'selected' : '' ?>>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="modalKategori" style="border: 2px solid #e2e8f0; border-radius: 14px; padding: 14px 18px; font-weight: 600;">
                            ...
                            <?php foreach ($daftar_kategori_filter as $k): ?>
                                <option value="<?= htmlspecialchars($k['Kategori_Properti']) ?>" <?= $kategori_filter == $k['Kategori_Properti'] ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($k['Kategori_Properti']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
    ...
    <script>
        ...
        function toggleStatus(id, currentStatus, nama) {
            ...
            Swal.fire({
                title: 'Ubah Status Properti?',
                text: 'Anda akan ' + actionText + ' properti "' + nama + '"',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    window.location.href = 'action_properti.php?aksi=toggle_status&id=' + id;
                }
            });
        }
        ...
        function hardDelete(id, nama) {
            Swal.fire({
                ...
                text: 'Properti "' + nama + '" akan dihapus PERMANEN dari database!',
                icon: 'error',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    window.location.href = 'action_properti.php?aksi=hard_delete&id=' + id;
                }
            });
        }
    ...
    <script>
        ...
        if ("<?= $_GET['status_sukses'] ?>" == 'tambah') msg = "Properti baru berhasil ditambahkan!";
        else if ("<?= $_GET['status_sukses'] ?>" == 'edit') msg = "Data properti berhasil diperbarui!";
        else if ("<?= $_GET['status_sukses'] ?>" == 'toggle_status') { msg = "Status properti berhasil diubah!"; t_title = "Status Diubah"; }
        else if ("<?= $_GET['status_sukses'] ?>" == 'hard_delete') { msg = "Properti berhasil dihapus permanen!"; t_title = "Hard Delete Berhasil"; }
