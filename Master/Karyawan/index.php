...
<head>
    ...
    <style>
        :root { --p-pink: #d83f67; --d-pink: #c73165; --s-pink: #fff5f6; --light-pink: #ffe4e9; --accent-pink: #ff6694; --text-dark: #1e1e24; --text-muted: #718096; --sidebar-bg: #ffffff; --body-bg: #f8fafc; --transition-3d: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); --transition-smooth: all 0.3s ease; }
        body { font-family: 'Plus Jakarta Sans', sans-serif; background-color: var(--body-bg); color: var(--text-dark); overflow-x: hidden; }
        .sidebar { width: 260px; height: 100vh; background: var(--sidebar-bg); position: fixed; top: 0; left: 0; border-right: 1px solid rgba(255, 236, 239, 0.8); display: flex; flex-direction: column; justify-content: space-between; padding: 30px 20px; z-index: 1040; transition: transform 0.45s cubic-bezier(0.175, 0.885, 0.32, 1.275); box-shadow: none; }
        ...
        .nav-link-custom { display: flex; align-items: center; justify-content: space-between; padding: 12px 18px; color: #4a5568; font-weight: 700; text-decoration: none; border-radius: 12px; font-size: 0.9rem; transition: var(--transition-smooth); }
        .nav-link-custom:hover, .nav-link-custom.active { background-color: var(--light-pink); color: var(--p-pink); transform: translateX(4px); }
        .submenu { list-style: none; padding-left: 20px; margin-top: 5px; display: none; transition: var(--transition-smooth); }
        ...
        .btn-logout { background: linear-gradient(135deg, var(--p-pink), var(--d-pink)); color: #ffffff; border: none; width: 100%; padding: 12px; border-radius: 12px; font-weight: 800; font-size: 0.85rem; transition: var(--transition-smooth); }
        .btn-logout:hover { transform: translateY(-2px); box-shadow: 0 6px 15px rgba(216, 63, 103, 0.2); }
        .sidebar-overlay { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(30, 30, 36, 0.45); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); z-index: 1035; opacity: 0; transition: opacity 0.35s ease; }
        .sidebar-overlay.show { display: block; opacity: 1; }
        .mobile-menu-btn { display: none; width: 44px; height: 44px; border-radius: 12px; background: #ffffff; border: 2px solid var(--light-pink); color: var(--p-pink); align-items: center; justify-content: center; font-size: 1.4rem; cursor: pointer; transition: var(--transition-smooth); flex-shrink: 0; box-shadow: 0 2px 8px rgba(0,0,0,0.04); }
        .mobile-menu-btn:hover { background: var(--s-pink); transform: scale(1.05); }
        .main-content { margin-left: 260px; padding: 40px; min-height: 100vh; transition: margin-left 0.35s ease; }
        .dashboard-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 35px; gap: 16px; flex-wrap: wrap; }
        .dashboard-header > div:first-child { flex: 1; min-width: 200px; }
        .profile-header-btn { width: 44px; height: 44px; border-radius: 50%; overflow: hidden; border: 2px solid #ffffff; cursor: pointer; transition: var(--transition-smooth); background: #ffffff; flex-shrink: 0; }
        ...
        .breadcrumb-custom { display: flex; align-items: center; gap: 8px; font-size: 0.8rem; color: var(--text-muted); margin-bottom: 12px; }
        .breadcrumb-custom a { color: var(--p-pink); text-decoration: none; font-weight: 700; transition: var(--transition-smooth); }
        .breadcrumb-custom a:hover { color: var(--d-pink); }
        .breadcrumb-custom .current { color: var(--text-dark); font-weight: 800; }
        .stats-scroll-wrapper { width: 100%; overflow-x: auto; overflow-y: hidden; padding-bottom: 15px; margin-bottom: 25px; scrollbar-width: thin; scrollbar-color: var(--p-pink) #f1f5f9; }
        ...
        .stat-icon { width: 48px; height: 48px; border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 1.4rem; transition: var(--transition-smooth); flex-shrink: 0; }
        ...
        .tab-filter-wrapper { display: flex; gap: 8px; margin-bottom: 25px; overflow-x: auto; padding-bottom: 6px; scrollbar-width: none; }
        .tab-filter-wrapper::-webkit-scrollbar { display: none; }
        .tab-filter-btn { padding: 10px 22px; border-radius: 12px; border: 1px solid rgba(255, 236, 239, 0.8); font-weight: 700; font-size: 0.82rem; cursor: pointer; transition: var(--transition-smooth); background: #ffffff; color: var(--text-muted); text-decoration: none; display: flex; align-items: center; gap: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.02); white-space: nowrap; flex-shrink: 0; }
        .tab-filter-btn:hover { border-color: var(--p-pink); color: var(--p-pink); transform: translateY(-2px); }
        .tab-filter-btn.active { background: linear-gradient(135deg, var(--p-pink), var(--d-pink)); color: #ffffff; border-color: var(--p-pink); box-shadow: 0 8px 20px rgba(216,63,103,0.2); }
        .tab-filter-btn .badge-count { background: rgba(0,0,0,0.05); padding: 2px 8px; border-radius: 50px; font-size: 0.7rem; font-weight: 800; color: var(--p-pink); }
        .tab-filter-btn.active .badge-count { background: rgba(255,255,255,0.2); color: #ffffff; }
        .btn-reg-header { background: linear-gradient(135deg, var(--p-pink), var(--d-pink)) !important; color: #ffffff !important; border-radius: 14px !important; padding: 12px 28px !important; font-weight: 800 !important; border: none !important; box-shadow: 0 8px 20px rgba(216, 63, 103, 0.25) !important; transition: var(--transition-smooth) !important; display: inline-flex; align-items: center; gap: 8px; font-size: 0.85rem; text-decoration: none; white-space: nowrap; }
        .btn-reg-header:hover { transform: translateY(-3px) scale(1.02); box-shadow: 0 12px 25px rgba(216, 63, 103, 0.35) !important; }
        ...
        .search-input-main { width: 100%; border: 2px solid #e2e8f0; border-radius: 14px; padding: 12px 18px 12px 44px; font-weight: 600; font-size: 0.9rem; color: #1e293b; transition: var(--transition-smooth); background: #ffffff; }
        .search-input-main:focus { outline: none; border-color: var(--p-pink); box-shadow: 0 0 0 4px rgba(216, 63, 103, 0.08); }
        .btn-filter-modal { background: var(--s-pink); color: var(--p-pink); border: 1.5px solid var(--light-pink); border-radius: 14px; padding: 12px 24px; font-weight: 700; font-size: 0.9rem; display: inline-flex; align-items: center; cursor: pointer; transition: var(--transition-smooth); white-space: nowrap; }
        .btn-filter-modal:hover { background: var(--light-pink); border-color: var(--p-pink); transform: translateY(-2px); box-shadow: 0 6px 16px rgba(216, 63, 103, 0.2); }
        .btn-search-icon { background: #ffffff; border: 2px solid #e2e8f0; border-radius: 14px; padding: 12px 16px; color: #94a3b8; cursor: pointer; transition: var(--transition-smooth); display: flex; align-items: center; justify-content: center; }
        ...
        .data-table { width: 100%; min-width: 950px; border-collapse: separate; border-spacing: 0; }
        ...
        .data-table tbody td { padding: 16px 20px; border: none; border-bottom: 1px solid #f1f5f9; vertical-align: middle; white-space: nowrap; font-size: 0.85rem; font-weight: 600; }
        .data-table tbody td:first-child { padding-left: 24px; font-weight: 800; color: var(--text-muted); }
        ...
        .data-table tbody tr.row-deleted { background-color: #fef2f2 !important; opacity: 0.85; }
        .data-table tbody tr.row-deleted:hover { background-color: #fee2e2 !important; }
        .avatar-default { width: 40px; height: 40px; border-radius: 12px; background: var(--s-pink); display: flex; align-items: center; justify-content: center; color: var(--p-pink); font-size: 1.2rem; flex-shrink: 0; overflow: hidden; border: 1.5px solid var(--light-pink); }
        .avatar-default img { width: 100%; height: 100%; object-fit: cover; }
        .nama-karyawan { font-weight: 800; color: var(--text-dark); font-size: 0.9rem; }
        .username-karyawan { font-size: 0.75rem; color: var(--text-muted); font-weight: 600; }
        .badge-role { font-size: 0.7rem; font-weight: 800; padding: 5px 12px; border-radius: 50px; display: inline-block; white-space: nowrap; text-transform: uppercase; letter-spacing: 0.5px; }
        ...
        .status-dot { display: inline-flex; align-items: center; gap: 6px; font-size: 0.8rem; font-weight: 700; }
        .status-dot .dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; }
        .status-dot .dot.aktif { background: #10b981; box-shadow: 0 0 0 3px rgba(16,185,129,0.25); }
        .status-dot .dot.nonaktif { background: #cbd5e1; }
        .status-dot .text-aktif { color: #10b981; }
        .status-dot .text-nonaktif { color: var(--text-muted); }
        .btn-action-circle { width: 34px; height: 34px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; transition: var(--transition-smooth); border: 1.5px solid #eef2f6; background: #ffffff; font-size: 0.85rem; text-decoration: none; margin: 0 2px; cursor: pointer; }
        ...
        .btn-action-toggle { color: var(--p-pink); border-color: #FFE4E9; }
        .btn-action-toggle:hover { background: var(--p-pink); color: #ffffff; transform: translateY(-2px); }
        ...
        .btn-action-hard { color: #dc2626; border-color: #fee2e2; }
        .btn-action-hard:hover { background: #dc2626; color: #ffffff; transform: translateY(-2px); }
        .pagination-wrapper { display: flex; justify-content: space-between; align-items: center; margin-top: 30px; padding: 20px 24px; background: #ffffff; border-radius: 20px; border: 1px solid rgba(255, 236, 239, 0.8); box-shadow: 0 4px 15px rgba(216, 63, 103, 0.04); flex-wrap: wrap; gap: 12px; }
        ...
        .page-link-pag { display: flex; align-items: center; justify-content: center; min-width: 40px; height: 40px; padding: 0 14px; border-radius: 12px; background: #ffffff; border: 2px solid #FFF5F7; color: #4a5568; font-weight: 700; font-size: 0.9rem; text-decoration: none; transition: var(--transition-smooth); }
        .page-link-pag:hover { background: var(--light-pink); border-color: var(--p-pink); color: var(--p-pink); transform: translateY(-2px); }
        .page-link-pag.active-pag { background: linear-gradient(135deg, var(--p-pink), var(--d-pink)) !important; color: #ffffff !important; border-color: var(--p-pink) !important; box-shadow: 0 4px 12px rgba(216, 63, 103, 0.3); }
        ...
        .form-control, .form-select { border-radius: 14px; padding: 12px 18px; border: 2px solid #eef2f6; background: #f8fafc; font-size: 14px; font-weight: 600; transition: var(--transition-smooth); color: var(--text-dark); }
        .form-control:focus, .form-select:focus { border-color: var(--p-pink); background: #ffffff; transform: translateY(-3px) scale(1.01); box-shadow: 0 12px 25px rgba(216, 63, 103, 0.15); outline: none; }
        .profile-preview-box { width: 90px; height: 90px; border-radius: 50%; overflow: hidden; border: 2.5px solid #eef2f6; background: #f8fafc; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(0,0,0,0.02); transition: var(--transition-smooth); }
        .profile-preview-box img { width: 100%; height: 100%; object-fit: cover; }
        .btn-pilih-foto { background: #ffffff; border: 1.5px solid var(--p-pink); color: var(--p-pink); font-weight: 700; border-radius: 10px; padding: 8px 18px; font-size: 0.85rem; transition: var(--transition-smooth); }
        .btn-pilih-foto:hover { background: var(--p-pink); color: #ffffff; transform: translateY(-2px); box-shadow: 0 6px 15px rgba(216, 63, 103, 0.15); }
        .btn-reg { background: linear-gradient(135deg, var(--p-pink), var(--d-pink)); color: white; border-radius: 16px; padding: 16px; font-weight: 800; border: none; width: 100%; transition: var(--transition-smooth); margin-top: 15px; font-size: 15px; box-shadow: 0 10px 25px rgba(216, 63, 103, 0.25); }
        .btn-reg:hover { transform: translateY(-4px) scale(1.02); box-shadow: 0 15px 35px rgba(216, 63, 103, 0.35); }
        .password-group { position: relative; transition: var(--transition-smooth); border-radius: 14px; }
        ...
        @media (max-width: 1199px) { .stats-row { gap: 12px; } .stat-card-item { min-width: 200px; } }
        @media (max-width: 992px) { .mobile-menu-btn { display: inline-flex; } .sidebar { transform: translateX(-100%); box-shadow: none; } .sidebar.mobile-open { transform: translateX(0); box-shadow: 10px 0 50px rgba(0,0,0,0.15); } .main-content { margin-left: 0; padding: 24px; } .dashboard-header { flex-wrap: wrap; gap: 12px; margin-bottom: 28px; } .dashboard-header h3 { font-size: 1.35rem; } }
        @media (max-width: 768px) { .main-content { padding: 18px; } .dashboard-header { margin-bottom: 22px; } .dashboard-header h3 { font-size: 1.15rem; } .dashboard-header p { font-size: 0.8rem; } .search-filter-bar { flex-direction: column; align-items: stretch; gap: 10px; } .search-form-flex { min-width: 100%; flex-wrap: wrap; } .search-input-wrapper { width: 100%; } .btn-reg-header { width: 100%; justify-content: center; } .pagination-wrapper { flex-direction: column; gap: 12px; padding: 16px; } .pagination-nav { justify-content: center; flex-wrap: wrap; } .stat-card-item { min-width: 170px; } }
        @media (max-width: 576px) { .main-content { padding: 14px; } .dashboard-header h3 { font-size: 1.05rem; } .data-table tbody td { padding: 12px 14px; } .data-table tbody td:first-child { padding-left: 16px; } .data-table tbody td:last-child { padding-right: 16px; } .btn-action-circle { width: 32px; height: 32px; font-size: 0.8rem; margin: 0 2px; } .page-link-pag { min-width: 36px; height: 36px; padding: 0 10px; font-size: 0.85rem; } .stat-val { font-size: 1.25rem; } .stat-icon { width: 40px; height: 40px; font-size: 1.2rem; } .modal-dialog { margin: 12px; } .modal-content { border-radius: 20px !important; } }
        @media (max-width: 375px) { .dashboard-header h3 { font-size: 0.95rem; } }
    </style>
</head>
<body>
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-menu-wrapper">
        ...
        <ul class="nav-menu">
            ...
            <li class="nav-item"><a href="../../Master/Karyawan/index.php" class="nav-link-custom active"><span><i class="bi bi-person-badge-fill me-2"></i> Kelola Karyawan</span></a></li>
            <li class="nav-item">
...
<div class="main-content">
    ...
    <div class="dashboard-header">
        <div class="d-flex align-items-center gap-3">
            <button class="mobile-menu-btn" onclick="toggleSidebar()" title="Menu" aria-label="Toggle Menu"><i class="bi bi-list"></i></button>
            <div><h3 class="fw-bold mb-1">Kelola Karyawan ✦</h3><p class="text-muted small mb-0">Kelola dan pantau hak akses data staf SpotLight Studio.</p></div>
        </div>
        <div class="d-flex align-items-center gap-3">
            ...
            <div class="profile-header-btn shadow-sm" onclick="bukaModalBiodata()" title="Klik untuk melihat Biodata Anda"><img src="<?= $foto_owner_src ?>" alt="Owner Profil"></div>
        </div>
    </div>

    <div class="stats-scroll-wrapper">
        <div class="stats-row">
            <div class="stat-card-item"><div class="card-3d" style="border-left: 3px solid var(--p-pink);"><div class="stat-card"><div class="stat-icon stat-icon-pink"><i class="bi bi-people-fill"></i></div><div class="stat-content"><div class="stat-title">Total Staf</div><div class="stat-val"><?= $stats['total'] ?></div><div class="stat-subtitle">Staf terdaftar</div></div></div></div></div>
            <div class="stat-card-item"><div class="card-3d" style="border-left: 3px solid #2563eb;"><div class="stat-card"><div class="stat-icon stat-icon-blue"><i class="bi bi-person-workspace"></i></div><div class="stat-content"><div class="stat-title">Admin</div><div class="stat-val"><?= $stats['admin'] ?></div><div class="stat-subtitle">Tim Pengelola</div></div></div></div></div>
            <div class="stat-card-item"><div class="card-3d" style="border-left: 3px solid var(--p-pink);"><div class="stat-card"><div class="stat-icon stat-icon-pink"><i class="bi bi-camera-fill"></i></div><div class="stat-content"><div class="stat-title">Fotografer</div><div class="stat-val"><?= $stats['fotografer'] ?></div><div class="stat-subtitle">Tim Kreatif</div></div></div></div></div>
            <div class="stat-card-item"><div class="card-3d" style="border-left: 3px solid #8b5cf6;"><div class="stat-card"><div class="stat-icon stat-icon-purple"><i class="bi bi-person-check-fill"></i></div><div class="stat-content"><div class="stat-title">Owner</div><div class="stat-val"><?= $stats['owner'] ?></div><div class="stat-subtitle">Hak Akses Penuh</div></div></div></div></div>
            <div class="stat-card-item"><div class="card-3d" style="border-left: 3px solid #10b981;"><div class="stat-card"><div class="stat-icon stat-icon-green"><i class="bi bi-check-circle-fill"></i></div><div class="stat-content"><div class="stat-title">Aktif</div><div class="stat-val"><?= $stats['aktif'] ?></div><div class="stat-subtitle">Staf aktif bekerja</div></div></div></div></div>
            <div class="stat-card-item"><div class="card-3d" style="border-left: 3px solid #d97706;"><div class="stat-card"><div class="stat-icon stat-icon-orange"><i class="bi bi-x-circle-fill"></i></div><div class="stat-content"><div class="stat-title">Nonaktif</div><div class="stat-val"><?= $stats['nonaktif'] ?></div><div class="stat-subtitle">Staf dinonaktifkan</div></div></div></div></div>
            <div class="stat-card-item"><div class="card-3d" style="border-left: 3px solid #dc2626;"><div class="stat-card"><div class="stat-icon stat-icon-red"><i class="bi bi-trash-fill"></i></div><div class="stat-content"><div class="stat-title">Dihapus</div><div class="stat-val"><?= $stats['dihapus'] ?></div><div class="stat-subtitle">Staf diarsipkan</div></div></div></div></div>
        </div>
    </div>
    ...
    <div class="search-filter-bar">
        ...
        <a href="tambah.php" class="btn btn-reg-header text-decoration-none"><i class="bi bi-plus-lg me-2"></i> Tambah Staf</a>
    </div>
    ...
    <div class="card-3d mb-4" style="padding: 24px;">
        <div class="table-scroll-wrapper">
            <table class="data-table">
                <thead>
                    <tr><th>No</th><th>NIK</th><th>Nama</th><th>Umur</th><th>Telepon</th><th>Kelamin</th><th>Peran</th><th>Status</th><th class="text-center">Aksi</th></tr>
                </thead>
                <tbody>
<?php
                    ...
                    <tr class="<?= $isDeleted ? 'row-deleted' : '' ?>">
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-3" style="cursor: pointer;" onclick="bukaDetailRow(this)"
                                ...
                                <div class="avatar-default"><img src="<?= $foto_karyawan_src ?>" alt="Avatar Staf"></div>
                                <div><div class="nama-karyawan"><?= htmlspecialchars($row['Nama_Karyawan']) ?></div><div class="username-karyawan">@<?= htmlspecialchars($row['Username_Karyawan']) ?></div></div>
                            </div>
                        </td>
                        <td><?= $umur ?></td>
                        ...
                        <td>
                            <?php if (!$isDeleted): ?>
                            <div class="status-dot"><span class="dot <?= $row['Status'] == 1 ? 'aktif' : 'nonaktif' ?>"></span><span class="<?= $row['Status'] == 1 ? 'text-aktif' : 'text-nonaktif' ?>"><?= $row['Status'] == 1 ? 'Aktif' : 'Nonaktif' ?></span></div>
                            <?php else: ?><span style="font-size: 0.75rem; color: #dc2626; font-weight: 800;">DIHAPUS</span><?php endif; ?>
                        </td>
                        <td>
                            <?php if (!$isDeleted): ?>
                                <a href="edit.php?id=<?= $row['ID_Karyawan'] ?>" class="btn-action-circle btn-action-edit" title="Edit Karyawan"><i class="bi bi-pencil"></i></a>
                                <button class="btn-action-circle btn-action-toggle" onclick="confirmSoftDelete(<?= $row['ID_Karyawan'] ?>, '<?= htmlspecialchars($row['Nama_Karyawan']) ?>')" title="Arsipkan Karyawan"><i class="bi bi-toggle2-on" style="font-size: 1.25rem;"></i></button>
                                <?php if (!$isOwnerSelf): ?>
                                    <button class="btn-action-circle btn-action-delete" onclick="confirmHardDelete(<?= $row['ID_Karyawan'] ?>, '<?= htmlspecialchars($row['Nama_Karyawan']) ?>')" title="Hapus Permanen"><i class="bi bi-trash"></i></button>
                                <?php else: ?>
                                    <button class="btn-action-circle btn-action-delete" style="opacity: 0.35; cursor: not-allowed;" disabled title="Tidak bisa hapus akun sendiri"><i class="bi bi-trash"></i></button>
                                <?php endif; ?>
                            <?php else: ?>
                                <button class="btn-action-circle btn-action-edit" style="opacity: 0.35; cursor: not-allowed;" disabled title="Pulihkan terlebih dahulu untuk mengedit"><i class="bi bi-pencil"></i></button>
                                <button class="btn-action-circle btn-action-restore" onclick="confirmRestore(<?= $row['ID_Karyawan'] ?>, '<?= htmlspecialchars($row['Nama_Karyawan']) ?>')" title="Pulihkan Karyawan"><i class="bi bi-toggle2-off" style="font-size: 1.25rem;"></i></button>
                                <?php if (!$isOwnerSelf): ?>
                                    <button class="btn-action-circle btn-action-hard" onclick="confirmHardDelete(<?= $row['ID_Karyawan'] ?>, '<?= htmlspecialchars($row['Nama_Karyawan']) ?>')" title="Hapus Permanen"><i class="bi bi-trash-fill"></i></button>
                                <?php else: ?>
                                    <button class="btn-action-circle btn-action-hard" style="opacity: 0.35; cursor: not-allowed;" disabled title="Tidak bisa hapus akun sendiri"><i class="bi bi-trash-fill"></i></button>
                                <?php endif; ?>
                            <?php endif; ?>
                        </td>
                    </tr>
<?php endwhile; else: ?>
                    <tr><td colspan="9" class="text-center py-5"><i class="bi bi-inbox" style="font-size: 2.5rem; color: #cbd5e1;"></i><p class="text-muted mt-2 fw-bold" style="font-size: 0.85rem;">Tidak ada data karyawan ditemukan</p></td></tr>
<?php endif; ?>
                </tbody>
            </table>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data">
          <div class="text-center mb-4">
            <div class="d-inline-block position-relative">
              <div class="profile-preview-box mx-auto"><img id="profile-preview-modal" src="<?= $foto_owner_src ?>" alt="Foto Profil"></div>
          ...
          <div class="mb-3"><label class="form-label">Nama Lengkap Anda<span class="required-star">*</span></label><input type="text" name="nama" id="inputNamaModal" class="form-control" placeholder="Masukkan nama lengkap Anda" value="<?= htmlspecialchars($nama_owner) ?>" required></div>
          <div class="mb-3"><label class="form-label">Nama Pengguna (Username)<span class="required-star">*</span></label><input type="text" name="username" id="inputUsernameModal" class="form-control" placeholder="Masukkan nama pengguna kustom" value="<?= htmlspecialchars($username_owner) ?>" required></div>
          <div class="mb-3"><label class="form-label">Alamat Email<span class="required-star">*</span></label><input type="email" name="email" class="form-control" placeholder="nama@email.com" value="<?= htmlspecialchars($email_owner) ?>" required></div>
          <div class="mb-3"><label class="form-label">Nomor Telepon<span class="required-star">*</span></label><input type="text" name="no_hp" id="inputHPModal" class="form-control" placeholder="Contoh: 08xxxxxxxxxx" value="<?= htmlspecialchars($d_profile['no_hp']) ?>" required></div>
...
<div class="modal fade" id="modalDetail" tabindex="-1" aria-hidden="true" style="backdrop-filter: blur(8px);">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0" style="border-radius: 28px; box-shadow: 0 20px 50px rgba(0,0,0,0.15); background: #ffffff;">
            ...
            <div class="modal-body px-4 pb-4 pt-3">
                <div class="text-center mb-4">
                    <div class="profile-preview-box mx-auto" style="width: 100px; height: 100px; border: 3px solid var(--s-pink);"><img id="d_foto" src="" alt="Foto Karyawan"></div>
                ...
                <div class="card-3d p-3 border-0 mb-4" style="border-radius: 20px; background-color: #f8fafc; box-shadow: none;">
                    <div class="row g-3">
                        <div class="col-6"><small class="text-muted d-block fw-bold" style="font-size: 0.7rem; text-transform: uppercase;">NIK</small><span class="fw-bold text-dark" style="font-size: 0.85rem;" id="d_nik"></span></div>
                        <div class="col-6"><small class="text-muted d-block fw-bold" style="font-size: 0.7rem; text-transform: uppercase;">Umur</small><span class="fw-bold text-dark" style="font-size: 0.85rem;" id="d_umur"></span></div>
                        ...
                        <div class="col-6 border-top pt-2"><small class="text-muted d-block fw-bold" style="font-size: 0.7rem; text-transform: uppercase;">Telepon</small><span class="fw-bold text-dark" style="font-size: 0.85rem;" id="d_hp"></span></div>
                        <div class="col-12 border-top pt-2"><small class="text-muted d-block fw-bold" style="font-size: 0.7rem; text-transform: uppercase;">Alamat Email</small><span class="fw-bold text-dark" style="font-size: 0.85rem;" id="d_email"></span></div>
                        <div class="col-12 border-top pt-2"><small class="text-muted d-block fw-bold" style="font-size: 0.7rem; text-transform: uppercase;">Alamat Lengkap</small><span class="fw-bold text-dark" style="font-size: 0.85rem;" id="d_alamat"></span></div>
                        <div class="col-12 border-top pt-2"><small class="text-muted d-block fw-bold" style="font-size: 0.7rem; text-transform: uppercase;">Status Staf</small><span class="fw-bold text-dark" style="font-size: 0.85rem;" id="d_status"></span></div>
                    </div>
                </div>
...
<script>
    ...
    function updateLiveClock() {
        ...
        if (clockEl) clockEl.innerText = `${dayName}, ${day} ${monthName} ${year} - ${hours}:${minutes}:${seconds} WIB`;
    }
    ...
    // PROFIL MODALS
    function bukaModalProfil() { new bootstrap.Modal(document.getElementById('modalGantiProfil')).show(); }
    function bukaModalBiodata() { new bootstrap.Modal(document.getElementById('modalLihatBiodata')).show(); }
    ...
    function bukaDetailRow(element) {
        ...
        if (d.role === 'Admin') { document.getElementById('d_role').className = 'badge bg-primary px-3 py-1 text-white text-uppercase'; }
        else if (d.role === 'Fotografer') { document.getElementById('d_role').className = 'badge bg-info px-3 py-1 text-white text-uppercase'; }
        else if (d.role === 'Owner') { document.getElementById('d_role').className = 'badge bg-dark px-3 py-1 text-white text-uppercase'; }
    ...
    function confirmSoftDelete(id, nama) {
        Swal.fire({ title: 'Arsipkan Karyawan? 📦', text: '"' + nama + '" akan diarsipkan dan dinonaktifkan sementara dari sistem.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d83f67', cancelButtonColor: '#718096', confirmButtonText: 'Ya, Arsipkan', cancelButtonText: 'Batal' }).then(r => { if (r.isConfirmed) window.location = 'action_karyawan.php?aksi=soft_delete&id=' + id; });
    }
...
<script>Swal.fire({ icon: 'success', title: 'Profil Diperbarui! 🎉', text: 'Informasi profil Anda berhasil disinkronkan ke seluruh sistem SpotLight.', confirmButtonColor: '#d83f67', confirmButtonText: 'Selesai' });</script>
<?php endif; ?>

<?php if(isset($error_profile) && $error_profile !== ""): ?>
<script>Swal.fire({ icon: 'error', title: 'Pembaruan Gagal! ❌', text: '<?= $error_profile ?>', confirmButtonColor: '#d83f67', confirmButtonText: 'Periksa Kembali' }).then(() => { new bootstrap.Modal(document.getElementById('modalGantiProfil')).show(); });</script>
<?php endif; ?>
