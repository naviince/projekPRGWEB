...
if (isset($_POST['tambah_karyawan'])) {
    ...
    if (empty($nik) || empty($nama) || empty($username) || empty($email) || empty($pass) || empty($dob) || empty($hp) || empty($alamat)) {
        $error_crud = "Seluruh kolom bertanda bintang wajib diisi!";
    } elseif (strlen($nik) < 16 || strlen($nik) > 20) {
        $error_crud = "Format NIK tidak valid!";
    } elseif ($umur < 17) {
        $error_crud = "Pendaftaran gagal! Umur karyawan minimal harus 17 tahun.";
    } else {
        // Cek duplikasi NIK, Email, Username, No HP [2]
        $sql_cek = "SELECT NIK, Email_Karyawan, Username_Karyawan, No_Hp FROM Karyawan WHERE NIK = ? OR Email_Karyawan = ? OR Username_Karyawan = ? OR No_Hp = ?";
        $stmt_cek = sqlsrv_query($conn, $sql_cek, array($nik, $email, $username, $hp));

        if ($stmt_cek && sqlsrv_has_rows($stmt_cek)) {
            $error_crud = "Data NIK, Email, Username, atau Nomor Telepon sudah digunakan!";
        } else {
            // Simpan langsung ke database Karyawan
            $sql_ins = "INSERT INTO Karyawan (NIK, Nama_Karyawan, Username_Karyawan, Email_Karyawan, Password_Karyawan, Jenis_Kelamin, Tanggal_Lahir, Role_Karyawan, No_Hp, Alamat, Status, Created_By) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)";
            $stmt_ins = sqlsrv_query($conn, $sql_ins, array($nik, $nama, $username, $email, $pass, $jk, $dob, $role, $hp, $alamat, $username_owner));
            if ($stmt_ins) {
                header("Location: index.php?status_sukses=tambah");
                exit();
            } else {
                $error_crud = "Gagal menyimpan data ke database!";
            }
        }
    }
}
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="dashboard-header" data-aos="fade-up">
            <div>
                <h3 class="fw-bold mb-1">Daftarkan Karyawan Baru ✦</h3>
                <p class="text-muted small mb-0">Lengkapi seluruh kolom di bawah ini secara akurat untuk pendaftaran staf baru.</p>
            </div>
        ...
        <!-- FORMULIR INPUT KARYAWAN -->
        <div class="card-3d">
            <form method="POST" onsubmit="return validasiUmur()">
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label">NIK<span class="required-star">*</span></label>
                        <!-- Sinkron dengan placeholder pendaftaran asli Anda -->
                        <input type="text" name="nik" id="inputNIK" class="form-control" placeholder="Masukkan NIK 16 digit" value="<?= @$_POST['nik'] ?>" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Nama Lengkap Karyawan<span class="required-star">*</span></label>
                        <input type="text" name="nama" id="inputNama" class="form-control" placeholder="Masukkan nama lengkap" value="<?= @$_POST['nama'] ?>" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Username Log-in<span class="required-star">*</span></label>
                        <input type="text" name="username" id="inputUsername" class="form-control" placeholder="Masukkan username" value="<?= @$_POST['username'] ?>" required>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="password" name="password" class="form-control" placeholder="Minimal 8 karakter (kombinasi)" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Nomor Telepon<span class="required-star">*</span></label>
                        <input type="text" name="no_hp" id="inputHP" class="form-control" value="<?= isset($_POST['no_hp']) ? @$_POST['no_hp'] : '+62 ' ?>" required>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="tanggal_lahir" id="t_dob" class="form-control" value="<?= @$_POST['tanggal_lahir'] ?>" required>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="role_karyawan" class="form-select" required>
                            <option value="" disabled selected>Pilih peran kerja</option>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="alamat" class="form-control" placeholder="Masukkan alamat domisili" value="<?= @$_POST['alamat'] ?>" required>
                    </div>
                </div>
                <button type="submit" name="tambah_karyawan" class="btn btn-reg shadow-sm py-3 mt-4">Daftarkan Karyawan Baru ✨</button>
            </form>
        </div>
    </div>
