...
// Halaman ini KHUSUS verifikasi Pelunasan - tolak kalau tipe-nya bukan Pelunasan (mis. DP)
if ($d_pembayaran['Tipe_Pembayaran'] !== 'Pelunasan') {
    header("Location: list.php?status=error&msg=" . urlencode("Pembayaran ini bukan Pelunasan dan tidak bisa diverifikasi dari halaman ini."));
    exit();
}
...
// SP ini sudah menangani (di level database, satu sumber logika, konsisten
// dengan verifikasi.php DP):
//   - Update status Pembayaran (Valid/Ditolak)
//   - Kalau Pelunasan Valid  -> Status_Order otomatis jadi 3 (Lunas)
//   - Kalau Pelunasan Ditolak -> Status_Pembayaran jadi Ditolak (2),
//     Status_Order TIDAK diubah oleh SP untuk kasus ini (order tetap di
//     status sebelumnya, karena DP-nya sendiri sudah terverifikasi lebih
//     dulu sebelum pelunasan bisa diupload -- lihat catatan di bawah).
...
if ($aksi === 'terima') {
    header("Location: list.php?status=sukses&msg=" . urlencode("Pembayaran Pelunasan diterima. Order sekarang LUNAS dan masuk ke Booking Customer untuk penjadwalan fotografer."));
} else {
    header("Location: list.php?status=sukses&msg=" . urlencode("Pembayaran Pelunasan ditolak. Customer harus upload ulang bukti pelunasan."));
}
exit();